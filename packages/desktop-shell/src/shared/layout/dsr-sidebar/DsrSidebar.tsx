...
import { useAppStore, type DsrScope } from '../../../store/useAppStore';
...
export const DsrSidebar: React.FC = () => {
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setDsrScope('project_drawings');
      setActiveWorkspaceId('drawing');
  ...
  const handleScopeSelect = (scope: DsrScope) => {
    // Switch to a global Drawing scope without bouncing to Project Home.
    setDsrScope(scope);
    setActiveWorkspaceId('drawing');
  ...
  const handleProjectSelect = (projectId: string, isTeam: boolean) => {
    setDsrScope(isTeam ? 'team_project_drawings' : 'project_drawings');
    ...
    >
      ...
      <Box sx={{ flex: 1, overflowY: 'auto' }}>
        ...
        <Box>
          <ScopeItem
            ...
            label="Drawing"
            active={dsrScope === 'global_drawings'}
            onClick={() => handleScopeSelect('global_drawings')}
            color="#2ecc71"
          />
          <ScopeItem
            icon={<FolderRoundedIcon />}
            label="Public Projects"
            active={dsrScope === 'global_projects'}
        ...
        <Box sx={{ mb: 2 }}>
          <ScopeItem
            ...
            active={dsrScope === 'my_public_drawings'}
            onClick={() => handleScopeSelect('my_public_drawings')}
            color="#9b59b6"
          />
          <ScopeItem
            icon={<LockRoundedIcon />}
            label="Private Drawing"
            active={dsrScope === 'my_private_drawings'}
            onClick={() => handleScopeSelect('my_private_drawings')}
            color="#e67e22"
          />
        </Box>
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {myProjects.map(p => (
            <ScopeItem
              ...
              label={p.name}
              active={p.id === activeProjectId && dsrScope === 'project_drawings'}
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {teamProjects.map(p => (
            <ScopeItem
              ...
              icon={<DescriptionRoundedIcon />}
              label={p.name}
              active={p.id === activeProjectId && dsrScope === 'team_project_drawings'}
      ...
      >
        ...
        <DialogActions sx={{ p: 2, pt: 0 }}>
          ...
          <Button onClick={handleCreateProject} disabled={isCreating || !newProjectName.trim()} variant="contained" sx={{ bgcolor: ACCENT, color: '#000', '&:hover': { bgcolor: '#80cbc4' } }}>
            {isCreating ? '作成中...' : '作成'}
          </Button>
        </DialogActions>
      </Dialog>
