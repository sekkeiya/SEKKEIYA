...
// Local Models の参照ソース（複数フォルダ）一覧・管理。「Local Models」スコープ時のみ表示。
...
function ModelSourceManager() {
  const sources = useModelSourcesStore(s => s.sources);
  const counts = useModelSourcesStore(s => s.counts);
  const subfolderCounts = useModelSourcesStore(s => s.subfolderCounts);
  ...
  const refresh = useModelSourcesStore(s => s.refresh);
  const addSourceViaDialog = useModelSourcesStore(s => s.addSourceViaDialog);
  const removeSource = useModelSourcesStore(s => s.removeSource);
  const toggleSource = useModelSourcesStore(s => s.toggleSource);
    ...
    <Box sx={{ mx: 1.5, mt: 0.5, mb: 1, p: 1, borderRadius: 2, bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 0.5, mb: 0.5 }}>
        ...
        <Tooltip title="フォルダを追加" placement="top">
          <IconButton size="small" onClick={() => addSourceViaDialog()} sx={{ color: 'rgba(255,255,255,0.6)', '&:hover': { color: LOCAL_ACCENT } }}>
          ...
          <React.Fragment key={src.id}>
            ...
            {tree.map(node => (
              <ModelSubfolderRow
                ...
                onClick={() => selectNode(src.id, node.path)}
              />
            ))}
          </React.Fragment>
        );
      ...
      <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', px: 0.5, mt: 0.5, lineHeight: 1.4 }}>
        対応形式: 3DM / BLEND / GLB / glTF
      </Typography>
    </Box>
  );
}

// サブフォルダ1行（インデントで階層を表現）。
function ModelSubfolderRow({ name, depth, count, active, onClick }: { name: string; depth: number; count: number; active: boolean; onClick: () => void; }) {
  return (
    <Box
      onClick={onClick}
      sx={{
        display: 'flex', alignItems: 'center', gap: 0.5, py: 0.4, borderRadius: 1.5, cursor: 'pointer',
        pl: `${10 + depth * 14}px`, pr: 0.75,
        bgcolor: active ? 'rgba(22,163,74,0.18)' : 'transparent',
        '&:hover': { bgcolor: active ? 'rgba(22,163,74,0.22)' : 'rgba(255,255,255,0.05)' },
      }}
    >
      <FolderOpenRoundedIcon sx={{ fontSize: 12, flexShrink: 0, color: active ? '#16a34a' : 'rgba(255,255,255,0.4)' }} />
      <Typography noWrap sx={{ flex: 1, minWidth: 0, fontSize: 11.5, fontWeight: active ? 600 : 500, color: active ? '#fff' : 'rgba(255,255,255,0.7)' }}>
        {name}
      </Typography>
      <Typography sx={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', flexShrink: 0 }}>{count}</Typography>
    </Box>
  );
}
...
function ModelSourceRow({ label, subtitle, count, active, enabled = true, missing = false, onClick, onToggle, onRemove }: ModelSourceRowProps) {
  return (
    <Box
      sx={{
    ...
    >
      ...
      <Box className="msrc-actions" sx={{ display: 'flex', opacity: 0, transition: 'opacity 0.15s' }}>
        {onToggle && (
...
export const ModelsSidebar: React.FC = () => {
  ...
  const handleScopeSelect = (scope: ModelsScope) => {
    setModelsScope(scope);
    setGlobalModelsHub(scope); // Switch to Hub view without kicking user to Project Home, preserving scope
  };

  const handleProjectSelect = (projectId: string, isTeam: boolean) => {
    setModelsScope(isTeam ? 'team_project_models' : 'project_models');
      ...
      <Box sx={{ flex: 1, overflowY: 'auto' }}>
        ...
        <Box>
          <ScopeItem
            ...
            label="Models"
            active={modelsScope === 'global_models' || modelsScope === 'global_following_models'}
            onClick={() => handleScopeSelect('global_following_models')}
            color="#2ecc71"
          />
          <ScopeItem
            icon={<FolderRoundedIcon />}
            label="Public Projects"
            active={modelsScope === 'global_projects' || modelsScope === 'global_following_projects'}
        ...
        <Box sx={{ mb: 2 }}>
          <ScopeItem 
            ...
            label="Public Models" 
            active={modelsScope === 'my_public_models'} 
            onClick={() => handleScopeSelect('my_public_models')} 
            color="#9b59b6"
          />
          <ScopeItem
            icon={<LockRoundedIcon />}
            label="Private Models"
            active={modelsScope === 'my_private_models'}
            onClick={() => handleScopeSelect('my_private_models')}
            color="#e67e22"
          />
          <ScopeItem
            icon={<FolderSpecialRoundedIcon />}
            label="Local Models"
            active={modelsScope === 'local_models'}
            onClick={() => handleScopeSelect('local_models')}
            color={LOCAL_ACCENT}
          />
          {modelsScope === 'local_models' && <ModelSourceManager />}
        </Box>
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {myProjects.map(p => (
            <ScopeItem 
              ...
              label={p.name} 
              active={p.id === activeProjectId && modelsScope === 'project_models'}
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {teamProjects.map(p => (
            <ScopeItem 
              ...
              icon={<ShapeLineRoundedIcon />} 
              label={p.name} 
              active={p.id === activeProjectId && modelsScope === 'team_project_models'}
