  ...
  return (
    <>
      ...
      <Popover
        ...
        slotProps={{ paper: { sx: { bgcolor: '#1a1c22', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', minWidth: 340, maxWidth: 420 } } }}
      >
        ...
        <Box sx={{ maxHeight: 360, overflowY: 'auto', p: 1 }}>
          {entries.map(([key, info]) => {
            ...
            return (
              <Box key={key} sx={{ p: 1, borderRadius: 1, '&:hover': { bgcolor: 'rgba(255,255,255,0.04)' } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography noWrap sx={{ fontSize: '0.8rem', fontWeight: 600 }}>{info.name}</Typography>
                    <Typography noWrap sx={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.5)' }}>
                ...
                {info.isNew && (
                  <Box sx={{ mt: 0.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography sx={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.5)' }}>公開設定:</Typography>
                    <ToggleButtonGroup
                      size="small" exclusive value={visOf(key)}
                      onChange={(_, v) => v && setVisByKey(prev => ({ ...prev, [key]: v }))}
                      sx={{ '& .MuiToggleButton-root': { color: 'rgba(255,255,255,0.6)', fontSize: '0.66rem', py: 0.1, px: 1, borderColor: 'rgba(255,255,255,0.15)' }, '& .Mui-selected': { color: '#fff !important', bgcolor: 'rgba(255,179,0,0.25) !important' } }}
                    >
