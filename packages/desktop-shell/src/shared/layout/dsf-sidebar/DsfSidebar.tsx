...
import { useAppStore, type DsfScope } from '../../../store/useAppStore';
...
export const DsfSidebar: React.FC = () => {
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setDsfScope('project_portfolios');
      setActiveWorkspaceId('portfolio');
  ...
  const handleScopeSelect = (scope: DsfScope) => {
    setDsfScope(scope);
    setActiveWorkspaceId('portfolio');
  ...
  const handleProjectSelect = (projectId: string, isTeam: boolean) => {
    setDsfScope(isTeam ? 'team_project_portfolios' : 'project_portfolios');
    ...
    >
      ...
      <Box sx={{ flex: 1, overflowY: 'auto' }}>
        ...
        <Box>
          <ScopeItem
            ...
            label="Portfolio"
            active={dsfScope === 'global_portfolios'}
            onClick={() => handleScopeSelect('global_portfolios')}
            color="#2ecc71"
          />
          <ScopeItem
            icon={<FolderRoundedIcon />}
            label="Public Projects"
            active={dsfScope === 'global_projects'}
        ...
        <Box sx={{ mb: 2 }}>
          <ScopeItem
            ...
            active={dsfScope === 'my_public_portfolios'}
            onClick={() => handleScopeSelect('my_public_portfolios')}
            color="#9b59b6"
          />
          <ScopeItem
            icon={<LockRoundedIcon />}
            label="Private Portfolio"
            active={dsfScope === 'my_private_portfolios'}
            onClick={() => handleScopeSelect('my_private_portfolios')}
            color="#e67e22"
          />
        </Box>
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {myProjects.map(p => (
            <ScopeItem
              ...
              label={p.name}
              active={p.id === activeProjectId && dsfScope === 'project_portfolios'}
        ...
        <Box sx={{ mb: 2, minWidth: 200 }}>
          ...
          {teamProjects.map(p => (
            <ScopeItem
              ...
              icon={<MenuBookRoundedIcon />}
              label={p.name}
              active={p.id === activeProjectId && dsfScope === 'team_project_portfolios'}
      ...
      >
        ...
        <DialogActions sx={{ p: 2, pt: 0 }}>
          ...
          <Button onClick={handleCreateProject} disabled={isCreating || !newProjectName.trim()} variant="contained" sx={{ bgcolor: ACCENT, color: 'var(--brand-fg)', '&:hover': { bgcolor: '#9575cd' } }}>
            {isCreating ? '作成中...' : '作成'}
          </Button>
        </DialogActions>
      </Dialog>
