...
interface CoreOrchestratorState {
  ...
  sendMessageToOrchestrator: (text: string, options?: { source?: OrchestratorSource; sessionId?: string; images?: OrchestratorImage[] }) => Promise<OrchestratorResponse>;
   ...
   * 'auto_models' = S.Models から自動選定して再配置、'manual_models' = S.Models ピッカーを開く。
...
export const useCoreOrchestrator = create<CoreOrchestratorState>((set, get) => {
  ...
  const runLoop = async (ctx: RunCtx): Promise<OrchestratorResponse | null> => {
    ...
    while (step < STEP_CAP) {
      ...
      const res = await agentTurnFn({ messages: history, model: ctx.model });
      if (aborted()) return null;
      ...
      // 固定の選択肢「S.Modelsから自動で選ぶ / S.Modelsから手動で選ぶ」を提示してターンを終える。
      ...
      if (noFurnitureResult && ctx.sessionId) {
        aiChatStore.addMessage({
          ...
          ui: {
            ...
            choices: [
              { id: 'auto_models', label: 'S.Modelsから自動で選ぶ', description: 'S.Models に登録済みのモデルからAIが自動で選定して配置します' },
              { id: 'manual_models', label: 'S.Modelsから手動で選ぶ', description: 'S.Models のモデル一覧から自分で選んで配置します' },
            ],
          },
        });
  ...
  return {
    ...
    sendMessageToOrchestrator: async (text, options) => {
      ...
      try {
        ...
        // source: 'task_auto_execute' はスケジュールタスク側で保存済みのため除外。
        if (currentUid && text.trim().length >= 4 && options?.source !== 'task_auto_execute') {
        ...
        // S.Layout レンダリング文脈（レンダー/レイアウト/間取り）。
        const isLayoutRenderRequest = /レンダ|render|レイアウト|間取り|ギャラリーに(入れ|追加)/i.test(text);
        const layoutRenderPlaybook = isLayoutRenderRequest ? LAYOUT_RENDER_PLAYBOOK : '';

        const systemPromptWithSite = baseSystemPrompt + calendarPlaybook + batchProgressPlaybook + aiTaskPlaybook + globalSessionPlaybook + gen3dPlaybook + materialGenPlaybook + knowledgePlaybook + layoutRenderPlaybook + rag.promptSection + furnitureContextSection + schedulesTasksContext + gen3dContext;

        const selectedModel = useAppStore.getState().selectedLlmModel;
        // Claude 系モデルのみ agentTurn（Anthropic）で処理。それ以外は legacy へ。
        ...
        const currentUserContent: string | ContentBlock[] = images.length > 0
          ? [
        ...
        const priorMsgs = sessionMsgs.slice(0, -1).slice(-MAX_HISTORY_MSGS);
        ...
        const result = await runLoop({
          sessionId, history, systemPromptWithSite, model: selectedModel, step: 0,
          source: options?.source, steps, citations: rag.citations,
        });
    ...
    resolveFurnitureSourceChoice: async (id, context) => {
      ...
      // ── 手動: S.Models 家具ピッカーを開く（ユーザーが選択→confirm で追加＆再配置）──
      if (id === 'manual_models') {
        const toolUseId = crypto.randomUUID();
        say('S.Models を開きました。配置するモデルを選んで確定してください。', {
        ...
        if (pid) {
          useAppStore.getState().setModelsScope('global_models');
          import('../features/launcher/launchWorkspace').then(({ launchWorkspace }) => {
            launchWorkspace({ appScope: '3dss', projectId: pid, workspaceId: 'models', workspaceName: 'S.Models' });
          });
        }
        return;
      }

      // ── 自動: S.Models のモデルから自動選定して再配置（LLM 非経由・1 パスで終わらせる）──
      ...
      set({ isProcessing: true, currentToolLabel: 'S.Models のモデルから自動配置しています…' });
      try {
        ...
        if (result.placedCount > 0) {
          ...
          say(`S.Models のモデルから ${result.placedCount} 点を自動配置しました。`);
        } else {
          // まだ 0 点＝S.Models に配置可能なモデルが無い。自動は再提示せず手動へ誘導し堂々巡りを断つ。
          say('S.Models に自動配置できるモデルが見つかりませんでした。モデルを手動で選ぶか、S.Models にアップロードしてください。', {
            kind: 'choices',
            toolUseId: crypto.randomUUID(),
            prompt: '次にどうしますか？',
            multiSelect: false,
...
async function dispatchAgentTool(name: string, input: any, dispatch: Function, sessionId?: string | null): Promise<any> {
  ...
  switch (name) {
    ...
    case 'add_furniture_to_project': {
      ...
      // 追加完了後、S.Models（プロジェクトモデル）を自動で開く。
      if (added > 0) {
        try {
          const appStore = useAppStore.getState();
          ...
          const { launchWorkspace } = await import('../features/launcher/launchWorkspace');
          await launchWorkspace({ appScope: '3dss', projectId: targetProjectId, workspaceId: 'models', workspaceName: 'S.Models' });
        } catch (e) {
          console.warn('[add_furniture_to_project] auto-navigate to S.Models failed:', e);
        }
      }
