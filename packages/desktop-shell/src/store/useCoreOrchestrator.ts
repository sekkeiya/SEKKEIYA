...
import { isMindMapMounted } from '../features/projects/chat/mindmapBridge';
import { getBoardContext } from '../features/projects/chat/boardContextBus';
import { getTurnDirectives, setTurnDirectives, APP_DIRECTIVE_MAP, type TurnDirectives } from './chatDirectives';
...
/**
 * 文脈判定（プレイブック注入・ツール silo）で遡るユーザー発話の数。
 * 「この会話で何をしているか」を見るためのもので、モデルへ送る履歴（MAX_HISTORY_MSGS）とは別。
 */
const MAX_CONTEXT_USER_MSGS = 15;
...
interface CoreOrchestratorState {
  ...
  sendMessageToOrchestrator: (text: string, options?: { source?: OrchestratorSource; sessionId?: string; images?: OrchestratorImage[]; docs?: OrchestratorDoc[]; hidden?: boolean; directives?: TurnDirectives }) => Promise<OrchestratorResponse>;
...
export const useCoreOrchestrator = create<CoreOrchestratorState>((set, get) => {
  ...
  return {
    ...
    sendMessageToOrchestrator: async (text, options) => {
      // 明示指定（/=アプリ、@=プロジェクト）をこのターンの指定として保持する。
      // 指定なしの送信でも {} で必ず上書きするので、前ターンの指定は残らない。
      setTurnDirectives(options?.directives ?? {});
      const aiChatStore = useAIChatStore.getState();
      ...
      try {
        ...
        // Research & Memo 文脈: タブを開いている間は常時注入（そこでの対話＝マップ/ボード作業）。
        // タブ外でもボード系キーワードで注入する。
        // ⚠️ チャットをポップアウトすると別ウィンドウ＝別コンテキストになり、本体の
        // useAppStore もブリッジのホスト登録も見えない。表示状態は boardContextBus 経由で
        // 本体から配信されたものを使う（ローカルの store を見ると必ず false になり、
        // ボード系ツールが silo で落ちて AI がマップを触れなくなる）。
        // 明示指定（/=アプリ、@=プロジェクト）。付いているときは文脈推測より優先する確定情報。
        const directives = getTurnDirectives();
        const directiveApp = directives.app ? APP_DIRECTIVE_MAP.get(directives.app) : undefined;

        // 会話全体のユーザー発話（＋今回の発話）。キーワード判定はこれに対して行う。
        // ⚠️ 今回の発話だけで見ると、2ターン目以降に語が消えた瞬間にツールが落ちて
        // AI が「ボードに書けないので文章で返す」状態になる（実際に踏んだ）。
        // サーバー siloTools と同じ「会話に一度でも出ていれば残す」方針に揃える。
        const conversationUserText = (() => {
          if (!sessionId) return text;
          const prior = useAIChatStore.getState().getMessagesForSession(sessionId)
            .filter(m => m.role === 'user')
            .slice(-MAX_CONTEXT_USER_MSGS)
            .map(m => m.text || '');
          return [...prior, text].join('\n');
        })();

        const board = getBoardContext();
        const isMemoTabOpen = board.open || useAppStore.getState().activeProjectTab === 'memo';
        const isResearchBoardContext = directives.app === 'mindmap' || directives.app === 'board'
          || isMemoTabOpen
          || /リサーチボード|リサーチキャンバス|Research\s*&?\s*Memo|メモボード|マインドマップ|論証グラフ|ボードに(置|貼|まとめ|整理|追加)|research.?board|mindmap/i.test(conversationUserText);
        // Research & Memo の既定ビューはマインドマップ。ノード画面を実際に開いているときだけ
        // 従来のボード・プレイブックを出す（両方出すと前置きが倍になり、どちらに置くかモデルが迷う）。
        // 明示指定があればそれが最優先。表示状態が取れないとき（タブ外からキーワードで来た等）は
        // 既定のマインドマップ側を採る。
        const showingNodeCanvas = directives.app === 'board' ? true
          : directives.app === 'mindmap' ? false
          : board.open
            ? board.view === 'canvas'
            : (useAppStore.getState().activeProjectTab === 'memo' && !isMindMapMounted());
        const researchBoardPlaybook = !isResearchBoardContext ? ''
          : showingNodeCanvas ? RESEARCH_BOARD_PLAYBOOK : MINDMAP_PLAYBOOK;

        // 明示指定をモデルにも一言で伝える（配線の本体は keepSilos とプレイブック切替。
        // これはモデルが迷ったときの決め手として後置する）。
        const directiveContext = (directiveApp || directives.project)
          ? '\n\n[明示指定]'
            + (directiveApp
              ? `\n- ユーザーはこのメッセージを「${directiveApp.label}」への指示だと明示しています。${directiveApp.hint}`
              : '')
            + (directives.project
              ? `\n- 対象プロジェクトは「${directives.project.name}」（projectId: ${directives.project.id}）。この指示のツール呼び出しではこのプロジェクトを対象にすること。`
              : '')
          : '';
        ...
        // 明示指定されたアプリのツール群は必ずモデルに届ける（silo 除外から保護。
        // クライアントの excludeSilos からも外す——keepSilos はサーバーのマージ後に効くが、
        // レイアウトスコープ等でこちら側が積んだ除外はここで戻すのが確実）。
        if (directiveApp) {
          for (const s of directiveApp.silos) {
            if (!keepSilos.includes(s)) keepSilos.push(s);
            const i = excludeSilos.indexOf(s);
            if (i >= 0) excludeSilos.splice(i, 1);
          }
        }
        ...
        const clientContext =
          ...
          schedulesTasksContext + gen3dContext + directiveContext;
...
// ─── マインドマップ（Research & Memo の既定ビュー）・プレイブック ─────────────
// Research & Memo は既定でマインドマップを開く。ここでの対話＝マップ作業なので、
// タブを開いている間ずっと注入する（キックオフの1メッセージだけでは会話が伸びると効かなくなる）。
const MINDMAP_PLAYBOOK = `
# マインドマップ伴走フロー（Research & Memo）

Research & Memo の既定の思考面は**マインドマップ**。ここでの作業は AI との対話が主役で、
マップは対話の内容が言語化・構造化されて定着する場所。
中心トピックから枝分かれする**木そのものが思考の構造**（親＝上位の論点 / 子＝それを支える具体）。

**最重要: マップ反映は自発的に行う。** ユーザーが「マップに置いて」「反映して」と言うのを待たない。
対話の各ターンで言語化できた論点・気づき・選択肢・決定は、その場で mindmap_add_topics で生やし、
置いたことを一言だけ報告する（「マップに置きました。違っていたら直します」程度）。違えば update/remove で直せばよい。
チャットは流れて消えるがマップは残る——**成果物はマップ、チャットは対話**。
要件整理・与条件・比較のような構造化情報をチャットで長々と出すくらいなら、同じ内容をトピックの木として置くこと。

## 使えるツール
- mindmap_get() — マップの現状（トピック一覧＝id/parentId/text/note/出典、関係線、まとめ）。**生やす/整理する前に必ず呼ぶ**。
- mindmap_add_topics({ topics, relations? }) — トピックを生やす。parent に既存 id か "#N"（今回の topics の添字）を渡して1回の呼び出しで部分木を組める。parent 省略は中心トピック直下。
- mindmap_update_topic({ id, text?, note?, link?, parent?, collapsed? }) — 見出しの推敲・note の追記・**parent で枝の付け替え（構造の整理）**。
- mindmap_remove_topics({ ids }) — 枝ごと削除（ユーザーが明確に求めたときのみ）。
- mindmap_connect_topics({ relations, removeRelationIds? }) — 別の枝どうしを関係線で結ぶ（木構造は変えない注釈）。
- research_board_create({ title }) — 新しいボードを作って切り替える（マップはボードの中身なので、これが新規マップの作り方）。テーマが明確に別のときだけ。安易に増やさない。
- research_board_generate_image({ prompts: [{prompt, caption?}] }) — コンセプト/ムードイメージを生成（約1分・最大4枚を1回で）。即時返却されるので完成を待たずに対話を続ける。
- search_knowledge({ query, category? }) — 外付け脳（S.Library/RAG）から根拠スニペットを検索。
- library_list() — S.Library の一覧（localId を refId に使う）。
- blog_list() / blog_get({ id }) — S.Blog 記事の一覧と本文(Markdown)全文＋coverUrl。
- gallery_query({ kind: 'image' }) — プロジェクトの既存画像資産（レンダー/パース/S.Image）。thumbnailUrl を image に使える。

## トピックの書き方
- **text は短い見出し**（体言止め・20字程度まで）。説明・根拠・詳細は **note** に入れる。長文を text に詰めない。
- 1トピック＝1つの意味。並列する要素は兄弟トピックに分ける。
- 深さは意味の階層に従う（親を読めば子の位置づけが分かる状態）。いきなり深く掘らず、まず論点を並べてから掘る。
- 出典のある内容は refType/refId/refTitle を付ける（原典に遡れる状態を保つ）。
- 画像は https の実URL のみ（data: URL・捏造URLは不可）。

## 対話の始め方（キックオフ）
- ユーザーがマップから対話を始めたら（【マインドマップ・キックオフ】指示を含む）、まず mindmap_get で現状を見る。
- 挨拶は一言で切り上げ、デザイナーの発想力・想像力を掻き立てる**具体的で答えやすい問い**を1〜2個だけ投げる。
- 良い問いの型: 体験の記憶（「その敷地で一番心が動いた瞬間は？」）/ 一場面の想像（「施主の休日の朝、どこで何をしていてほしい？」）/
  対比（「開くべき方向と、閉じて守るべきものは？」）/ 参照（「空気感が近いと感じた空間は？」）。抽象論・一般論の質問はしない。
- 中心トピックしか無い状態で、いきなり一般論の枝（「コンセプト」「ゾーニング」等の空っぽの見出し）を大量生成しない。ユーザーの言葉が出てから、その言葉でトピックを作る。

## 対話の作法（深掘り → 言語化 → 構造化 → 根拠づけ → 可視化）
1. **深掘り**: ユーザーの言葉が抽象的なうちは問いで具体化する（誰のため？敷地の何が効く？なぜその素材？）。問いの過程で出た具体は、その都度トピック化してよい。
2. **言語化**: 輪郭が出た論点・キーワードは**その場で**トピックに置く（「置きましょうか？」と許可を求めない）。置いたら一言だけ報告し、違っていれば update/remove で直す。
3. **構造化**: 対話が進んだら親子関係を見直す。同じ階層に並びすぎていたら束ねる親を作り、掘るべき枝は mindmap_update_topic の parent で付け替える。
   別の枝どうしに関係（トレードオフ・同じ根拠・前提）が見えたら mindmap_connect_topics で結び、text に関係の一言を入れる。
4. **根拠づけ**: 主張には search_knowledge / library_list / blog_get で根拠を探し、その要点を子トピック（refTitle・refId 付き）で足す。根拠が見つからない主張は「まだ仮説」と note に明示する。
5. **可視化**: 言葉で伝わりにくい空気感・素材感・光の方向性が出てきたら research_board_generate_image でイメージを生成して見せ、ユーザーの反応（どこが良い/違う）からさらに深掘りする。

## 原則
- 毎ターン、そのターンで言語化できた論点・決定・根拠を能動的にマップへ反映する（置いてから一言報告。事前の許可取りはしない）。
- **チャット本文にマインドマップの木を文字で描かない**（\`├─\` や箇条書きでの疑似マップは禁止）。構造を示すなら mindmap_add_topics で実物を置く。
  マップの内容を語るときは、置いた結果を一言で要約するだけにする。
- 能動的に置くのは「対話に出た内容」と「出典のある根拠」。ユーザーの言葉にない論点をゼロから大量創作してマップを埋めない。
- ユーザーが書いたトピックの text 変更・削除は、必ず事前に合意を取る（AIが置いたトピックは自由に直してよい）。
- 出典の捏造は厳禁。refTitle/refId/画像URL は search_knowledge・library_list・blog_get・gallery_query の実データのみ使う。
- ユーザーが「ノード画面」「論証グラフ」「カード」と明示したときだけ research_board_* を使う（既定はマインドマップ）。
`;

// ─── リサーチボード（Research & Memo のノード画面）・プレイブック ─────────────
// ノード画面は既定ビューから外れた（既存ボードの閲覧・編集用）。ユーザーが明示的に
// ノード画面／論証グラフを触っているときだけ注入する。
const RESEARCH_BOARD_PLAYBOOK = `
# リサーチボード伴走フロー（Research & Memo のノード画面）
...
async function dispatchAgentTool(name: string, input: any, dispatch: Function, sessionId?: string | null): Promise<any> {
  ...
  const resolveProjectId = (inputPid?: string): string | undefined => {
    // ユーザーが @ でプロジェクトを明示したターンは、それが最優先
    // （セッションよりも強い、そのメッセージ限りの一時指定）。
    const dirProject = getTurnDirectives().project;
    if (dirProject?.id) return dirProject.id;
