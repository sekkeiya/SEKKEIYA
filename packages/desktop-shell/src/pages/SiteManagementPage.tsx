...
const STATUS_META: Record<Status, { label: string; color: string; bg: string }> = {
  ...
  draft: { label: '未公開', color: 'rgba(255,255,255,0.6)', bg: 'rgba(255,255,255,0.08)' },
  none: { label: '未作成', color: 'rgba(255,255,255,0.4)', bg: 'rgba(255,255,255,0.05)' },
};
  ...
  return (
    <Box sx={{ flex: 1, p: { xs: 2.5, md: 5 }, display: 'flex', flexDirection: 'column', bgcolor: 'background.default', overflowY: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            {logoUrl
              ...
              : <Box sx={{ width: 38, height: 38, borderRadius: 1.5, bgcolor: 'rgba(255,255,255,0.06)', border: `1px solid ${BRAND.line}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><PublicRoundedIcon sx={{ color: '#00BFFF' }} /></Box>}
            <Typography variant="h5" sx={{ fontWeight: 800, color: '#fff' }}>サイト管理</Typography>
          </Box>
          <Button variant="contained" startIcon={<AddRoundedIcon />} onClick={() => setCreateOpen(true)}
            sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: '#fff', fontWeight: 700, textTransform: 'none', borderRadius: 2, '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' } }}>
            新規プロジェクト
          </Button>
        </Box>
        ...
        {loading ? (
          ...
          <>
            ...
            <Paper sx={{ p: 1.25, px: 2, mb: 2.5, bgcolor: 'rgba(187,143,206,0.08)', border: '1px solid rgba(187,143,206,0.25)', borderRadius: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <Box sx={{ width: 34, height: 34, borderRadius: 1.5, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'rgba(255,255,255,0.06)', flexShrink: 0 }}>
                {logoUrl ? <Box component="img" src={logoUrl} alt="" sx={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <PersonRoundedIcon sx={{ color: '#bb8fce', fontSize: '1.2rem' }} />}
              </Box>
              <Box sx={{ minWidth: 0, flex: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Typography noWrap sx={{ fontWeight: 700, color: '#fff', fontSize: '0.9rem' }}>{accountName}</Typography>
                  <Tooltip title="名前を変更"><IconButton size="small" onClick={() => openRename('account', uid, accountName)} sx={{ color: 'rgba(255,255,255,0.4)', p: 0.25, '&:hover': { color: '#fff' } }}><EditRoundedIcon sx={{ fontSize: '0.85rem' }} /></IconButton></Tooltip>
                  <Chip label="アカウント" size="small" sx={{ height: 18, fontSize: '0.62rem', fontWeight: 800, color: '#bb8fce', bgcolor: 'rgba(187,143,206,0.16)' }} />
                  <StatusBadge status={statusOf(accountSite)} />
                  {(() => { const v = visibilityOf(accountSite); return v ? <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.4, color: v.icon === 'private' ? '#f6c453' : 'rgba(255,255,255,0.6)', fontSize: '0.7rem' }}>{v.icon === 'private' ? <LockRoundedIcon sx={{ fontSize: '0.8rem' }} /> : <PublicRoundedIcon sx={{ fontSize: '0.8rem' }} />}{v.label}</Box> : null; })()}
                </Box>
                {statusOf(accountSite) === 'published' && accountSite?.publish?.slug && (
                  <Typography noWrap sx={{ color: '#7fd1ff', fontSize: '0.72rem' }}>{publicUrl(accountSite.publish.slug)}</Typography>
                )}
              </Box>
              <Button size="small" onClick={() => setCurrentMainView('my-site')} startIcon={<LaunchRoundedIcon sx={{ fontSize: '0.95rem' }} />}
                sx={{ color: 'rgba(255,255,255,0.7)', textTransform: 'none', border: `1px solid ${BRAND.line}`, borderRadius: 1.5, '&:hover': { color: '#fff' } }}>編集</Button>
              <Button size="small" disabled={!accountSite} onClick={() => accountSite && setTarget({ source: { kind: 'account', id: uid }, site: accountSite, displayName: username || accountName })}
                ...
                sx={{ bgcolor: '#00BFFF', color: '#000', fontWeight: 800, textTransform: 'none', borderRadius: 1.5, '&:hover': { bgcolor: '#4facfe' }, '&.Mui-disabled': { bgcolor: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.3)' } }}>公開設定</Button>
            </Paper>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1, flexWrap: 'wrap', gap: 1 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
                {TABS.map((t, i) => (
                  <React.Fragment key={t.key}>
                    ...
                    <Box onClick={() => setFilter(t.key)} sx={{ cursor: 'pointer', fontSize: '0.82rem', fontWeight: filter === t.key ? 800 : 500, color: filter === t.key ? '#00BFFF' : 'rgba(255,255,255,0.6)', '&:hover': { color: '#fff' } }}>
              ...
              <TextField value={query} onChange={e => setQuery(e.target.value)} placeholder="サイトを検索" size="small"
                ...
                sx={{ width: 230, '& .MuiOutlinedInput-root': { borderRadius: 2, bgcolor: 'rgba(255,255,255,0.04)', '& fieldset': { borderColor: 'rgba(255,255,255,0.14)' }, '&:hover fieldset': { borderColor: 'rgba(255,255,255,0.3)' }, '&.Mui-focused fieldset': { borderColor: '#00BFFF' } } }} />
            </Box>
            ...
            <Paper sx={{ bgcolor: BRAND.panel, border: `1px solid ${BRAND.line}`, borderRadius: 2, overflow: 'hidden' }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: COLS, gap: 1.5, alignItems: 'center', px: 1.5, py: 1, borderBottom: `1px solid ${BRAND.line}`, bgcolor: 'rgba(255,255,255,0.03)', fontSize: '0.76rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)' }}>
                <Checkbox size="small" checked={allChecked} indeterminate={!allChecked && someChecked} onChange={toggleAll} sx={{ p: 0, color: 'rgba(255,255,255,0.4)', '&.Mui-checked': { color: '#00BFFF' }, '&.MuiCheckbox-indeterminate': { color: '#00BFFF' } }} />
              ...
              {/* データ行 */}
              {pageItems.length === 0 ? (
                <Box sx={{ py: 6, textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>条件に一致するサイトはありません。</Box>
              ) : pageItems.map(p => {
                ...
                return (
                  <Box key={p.id} sx={{ display: 'grid', gridTemplateColumns: COLS, gap: 1.5, alignItems: 'center', px: 1.5, py: 1, borderBottom: `1px solid ${BRAND.line}`, transition: 'background 0.12s', bgcolor: checked ? 'rgba(0,191,255,0.06)' : 'transparent', '&:hover': { bgcolor: 'rgba(255,255,255,0.03)' }, '&:last-of-type': { borderBottom: 'none' } }}>
                    <Checkbox size="small" checked={checked} onChange={() => toggleOne(p.id)} sx={{ p: 0, color: 'rgba(255,255,255,0.35)', '&.Mui-checked': { color: '#00BFFF' } }} />
                    {/* 名前 */}
                    <Box sx={{ ...cell, gap: 1.25 }}>
                      <Box sx={{ width: 28, height: 28, borderRadius: 1, overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'rgba(255,255,255,0.05)', border: `1px solid ${BRAND.line}` }}>
                      ...
                      <Box sx={{ minWidth: 0 }}>
                        <Typography onClick={() => openProject(p.id)} noWrap sx={{ fontWeight: 700, color: '#fff', fontSize: '0.86rem', cursor: 'pointer', '&:hover': { color: '#00BFFF' } }}>{p.name}</Typography>
                        {p.isTeam && <Typography sx={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.06em' }}>TEAM</Typography>}
                      </Box>
                    </Box>
                    {/* 状況 */}
                    ...
                    <Box sx={cell}>
                      {(() => { const v = visibilityOf(site); return v
                        ? <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.4, color: v.icon === 'private' ? '#f6c453' : 'rgba(255,255,255,0.65)', fontSize: '0.74rem' }}>
                    ...
                    <Box sx={{ ...cell, gap: 0.25 }}>
                      {url ? (
                        <>
                          <Typography noWrap sx={{ color: '#7fd1ff', fontSize: '0.74rem', minWidth: 0 }}>{url.replace('https://', '')}</Typography>
                          <Tooltip title="コピー"><IconButton size="small" onClick={() => copy(url)} sx={{ color: 'rgba(255,255,255,0.4)', p: 0.25 }}><ContentCopyRoundedIcon sx={{ fontSize: '0.8rem' }} /></IconButton></Tooltip>
                          <Tooltip title="開く"><IconButton size="small" onClick={() => window.open(url, '_blank')} sx={{ color: 'rgba(255,255,255,0.4)', p: 0.25 }}><OpenInNewRoundedIcon sx={{ fontSize: '0.8rem' }} /></IconButton></Tooltip>
                        </>
                      ) : <Typography sx={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.78rem' }}>—</Typography>}
                    </Box>
                    {/* 更新日時 */}
                    <Box sx={{ ...cell, color: 'rgba(255,255,255,0.5)', fontSize: '0.74rem' }}>{fmtDateTime(site?.updatedAt || p.lastModifiedAt)}</Box>
                    {/* 操作 */}
                    <Box sx={{ ...cell, justifyContent: 'flex-end', gap: 0.25 }}>
                      <Tooltip title={site ? 'サイトを開く' : 'サイトを作成'}>
                        <IconButton size="small" onClick={() => openProject(p.id)} sx={{ color: 'rgba(255,255,255,0.6)', '&:hover': { color: '#fff' } }}><LaunchRoundedIcon sx={{ fontSize: '1.05rem' }} /></IconButton>
                      </Tooltip>
                      <Tooltip title={site ? '公開設定' : 'まずサイトを作成してください'}>
                        <span>
                          <IconButton size="small" disabled={!site} onClick={() => site && setTarget({ source: { kind: 'project', id: p.id }, site, displayName: p.name })}
                            sx={{ color: status === 'published' ? '#43e97b' : '#00BFFF', '&.Mui-disabled': { color: 'rgba(255,255,255,0.2)' } }}><PublicRoundedIcon sx={{ fontSize: '1.05rem' }} /></IconButton>
                        </span>
                      </Tooltip>
                      <IconButton size="small" onClick={e => { setMenuAnchor(e.currentTarget); setMenuProjectId(p.id); }} sx={{ color: 'rgba(255,255,255,0.5)', '&:hover': { color: '#fff' } }}><MoreVertRoundedIcon sx={{ fontSize: '1.1rem' }} /></IconButton>
                    </Box>
                  </Box>
                );
              })}
      ...
      <Menu anchorEl={menuAnchor} open={!!menuAnchor} onClose={() => setMenuAnchor(null)}
        slotProps={{ paper: { sx: { bgcolor: '#1a1f2a', color: '#fff', border: `1px solid ${BRAND.line}`, minWidth: 200 } } }}>
        {menuProject && <MenuItem onClick={() => { openProject(menuProject.id); setMenuAnchor(null); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><LaunchRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>{menuSite ? '開く' : 'サイトを作成'}</MenuItem>}
        {menuProject && menuPerm.canEdit && <MenuItem onClick={() => { const p = menuProject; setMenuAnchor(null); openRename('project', p.id, p.name); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><EditRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>名前を変更</MenuItem>}
        ...
        {menuProject && menuProject.isTeam && menuPerm.canManage && <MenuItem onClick={() => { const p = menuProject; setMenuAnchor(null); setMemberTarget(p); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><GroupAddRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>メンバー管理</MenuItem>}
        {menuProject && menuProject.isTeam && menuPerm.isMember && <MenuItem onClick={() => { const p = menuProject; setMenuAnchor(null); setConfirm({ title: 'MYプロジェクトとして複製', message: `「${p.name}」を自分の個人プロジェクトとして複製します（サイト構成を引き継ぎ、未公開の下書きになります）。`, confirmLabel: '複製する', onConfirm: () => duplicateToMy(p) }); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><ContentCopyRoundedIcon sx={{ fontSize: '1.05rem' }} /></ListItemIcon>MYプロジェクトとして複製</MenuItem>}

        {menuSite?.publish?.status === 'published' && <MenuItem onClick={() => { window.open(publicUrl(menuSite.publish!.slug), '_blank'); setMenuAnchor(null); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><OpenInNewRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>ブラウザで開く</MenuItem>}
        {menuSite?.publish?.status === 'published' && menuProject && menuPerm.canManage && <MenuItem onClick={() => { const p = menuProject; setMenuAnchor(null); setConfirm({ title: '公開を停止', message: `「${p.name}」の公開を停止します。データは残ります。`, confirmLabel: '公開を停止', onConfirm: () => unpublish(p) }); }} sx={{ fontSize: '0.85rem' }}><ListItemIcon sx={{ color: 'rgba(255,255,255,0.7)', minWidth: 32 }}><VisibilityOffRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>公開を停止</MenuItem>}
        {menuSite && menuProject && menuPerm.canManage && <MenuItem onClick={() => { const p = menuProject; setMenuAnchor(null); setConfirm({ title: 'サイトを削除', message: `「${p.name}」のサイトを削除します（プロジェクトは残ります）。この操作は取り消せません。`, confirmLabel: '削除する', danger: true, onConfirm: () => deleteSite(p) }); }} sx={{ fontSize: '0.85rem', color: '#fa9bb4' }}><ListItemIcon sx={{ color: '#fa9bb4', minWidth: 32 }}><DeleteOutlineRoundedIcon sx={{ fontSize: '1.1rem' }} /></ListItemIcon>サイトを削除</MenuItem>}
      ...
      <Dialog open={!!confirm} onClose={() => !busy && setConfirm(null)}
        ...
        <DialogActions sx={{ p: 2, pt: 0 }}>
          ...
          <Button onClick={runConfirm} disabled={busy} variant="contained" sx={{ bgcolor: confirm?.danger ? '#ef4444' : '#00BFFF', color: confirm?.danger ? '#fff' : '#000', fontWeight: 800, '&:hover': { bgcolor: confirm?.danger ? '#dc2626' : '#4facfe' } }}>
      ...
      <Dialog open={!!renameTarget} onClose={() => !renaming && setRenameTarget(null)}
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label="名前" fullWidth variant="outlined" value={renameValue}
            ...
            sx={{ mt: 1, '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: 'rgba(255,255,255,0.2)' }, '&:hover fieldset': { borderColor: 'rgba(255,255,255,0.4)' }, '&.Mui-focused fieldset': { borderColor: '#00BFFF' } } }} />
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setRenameTarget(null)} disabled={renaming} sx={{ color: 'rgba(255,255,255,0.7)' }}>キャンセル</Button>
      ...
      <Dialog open={createOpen} onClose={() => !creating && setCreateOpen(false)}
        ...
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setCreateOpen(false)} disabled={creating} sx={{ color: 'rgba(255,255,255,0.7)' }}>キャンセル</Button>
