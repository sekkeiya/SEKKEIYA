...
const Login: React.FC = () => {
  ...
  const web = isWeb();
  // Web: Firebase の popup を使うため client id/secret は不要（常に利用可）。
  // iOS: PKCE のみ（client secret なし）。Desktop: loopback。
  const isGoogleAvailable = web
    ? true
    : isIos
      ? Boolean(GOOGLE_IOS_CLIENT_ID)
      : Boolean(GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET);

  // Web（ブラウザ）：標準の Firebase popup サインイン。
  ...
  const handleGoogleLogin = web ? handleGoogleLoginWeb : (isIos ? handleGoogleLoginIos : handleGoogleLoginDesktop);
