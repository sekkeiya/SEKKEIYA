 ...
 */
class ThreeDMLoader extends Loader {
	...
	constructor(manager) {
		...
		// Use only this instance-level cache. Keys are URL strings.
	...
	load(url, onLoad, onProgress, onError) {
		...
		loader.setResponseType(this.responseType || 'arraybuffer');
		...
		const handleError = (error) => {
			...
			if (error?.target?.response) {
				const text = new TextDecoder().decode(error.target.response);
				console.log("🔥 エラーレスポンスの中身 →", text);

				if (text.startsWith("<")) {
					console.error("⚠️ 取得したファイルは HTML です。URL を確認してください。");
				}
			} else {
				console.log("error.target が無いので、ファイルレスポンスではない可能性があります。");
			}
		...
		return loader.load(
			url,
			(buffer) => {
				// If already loading/loaded this URL, reuse the Promise.
				if (this._taskCache.has(cacheKey)) {
					const cached = this._taskCache.get(cacheKey);
					cached.promise.then(onLoad).catch(handleError);
					return cached.promise;
				}

				const objectPending = this.decodeObjects(buffer, url)
					.then(result => {
				...
				this._taskCache.set(cacheKey, { url, promise: objectPending });
	...
	decodeObjects(buffer, url) {
		...
		const taskCost = buffer?.byteLength || 0;
		...
		const objectPending = this._getWorker(taskCost)
			.then((_worker) => {
				...
				return new Promise((resolve, reject) => {
					...
					// Always transfer a fresh copy; the original may be needed elsewhere.
					const transferable = buffer.slice(0);
					worker._taskCosts[taskID] = taskCost;
					worker.postMessage({ type: 'decode', id: taskID, buffer: transferable }, [transferable]);
				});
		...
		// Remove task from the task list when done.
		objectPending
			.catch(() => true)
			.then(() => {
				if (worker && taskID) {
	...
	_createMaterial(material, renderEnvironment) {
		...
		for (let i = 0; i < material.textures.length; i++) {
			...
			if (texture.image !== null) {
				...
				switch (texture.type) {
					case 'Bump': mat.bumpMap = map; break;
					case 'Diffuse': mat.map = map; break;
					case 'Emap': mat.envMap = map; break;
					case 'Opacity': mat.transmissionMap = map; break;
					case 'Transparency': mat.alphaMap = map; mat.transparent = true; break;
					case 'PBR_Alpha': mat.alphaMap = map; mat.transparent = true; break;
					case 'PBR_AmbientOcclusion': mat.aoMap = map; break;
					case 'PBR_Anisotropic': mat.anisotropyMap = map; break;
					case 'PBR_BaseColor': mat.map = map; break;
					case 'PBR_Clearcoat': mat.clearcoatMap = map; break;
					case 'PBR_ClearcoatBump': mat.clearcoatNormalMap = map; break;
					case 'PBR_ClearcoatRoughness': mat.clearcoatRoughnessMap = map; break;
					case 'PBR_Displacement': mat.displacementMap = map; break;
					case 'PBR_Emission': mat.emissiveMap = map; break;
					case 'PBR_Metallic': mat.metalnessMap = map; break;
					case 'PBR_Roughness': mat.roughnessMap = map; break;
					case 'PBR_Sheen': mat.sheenColorMap = map; break;
					case 'PBR_Specular': mat.specularColorMap = map; break;
					case 'PBR_Subsurface': mat.thicknessMap = map; break;
					default:
						this.warnings.push({
	...
	_initLibrary() {
		if (!this.libraryPending) {
			...
			const jsContent = new Promise((resolve, reject) => {
				jsLoader.load('rhino3dm.min.js', resolve, undefined, reject);
			});
...
function Rhino3dmWorker() {
	...
	function decodeObjects(rhino, buffer) {
		...
		// Render Environment for materials
