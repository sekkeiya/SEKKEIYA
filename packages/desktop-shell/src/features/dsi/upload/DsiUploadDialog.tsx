...
import { dsiUploadService } from './dsiUploadService';
import { DSI_CATEGORIES, type DsiCategory } from '../store/useDsiStore';

const ACCENT = '#ec407a';
const ACCENT_HOVER = '#f48fb1';
// 動画は S.Movie 側で管理するため、S.Image のアップロードは画像のみ受け付ける。
...
export const DsiUploadDialog: React.FC<DsiUploadDialogProps> = ({ open, onClose, projectId, defaultParentSetId = null, sets }) => {
  ...
  const acceptFile = (f: File | undefined | null) => {
    ...
    if (!ALLOWED_EXT.includes(ext)) {
      setError('画像（PNG/JPG/WebP/GIF）のみアップロードできます。動画は S.Movie で管理してください。');
      return;
    }
    setError(null);
    setFile(f);
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pb: 1 }}>画像をアップロード</DialogTitle>
      <DialogContent>
        {/* Drop zone */}
        <Box
        ...
        >
          ...
          {file ? (
            ...
            <>
              ...
              <Typography sx={{ fontSize: 11, color: 'rgb(var(--brand-fg-rgb) / 0.4)', mt: 0.5 }}>画像 PNG / JPG / WebP / GIF</Typography>
            </>
          )}
        </Box>
