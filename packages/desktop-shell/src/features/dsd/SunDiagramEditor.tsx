...
import { useDsdStore, type DsdEditorTab } from './store/useDsdStore';
import { SunDiagramCanvas, type SunDiagramCanvasHandle } from './canvas/SunDiagramCanvas';
import { calcSunriseSunset } from './canvas/sunMath';
...
export const SunDiagramEditor: React.FC = () => {
  ...
  const canvasRef = useRef<SunDiagramCanvasHandle>(null);
  ...
  const { style, setStyle, diagramTitle, isDrawingPolygon, editorTab, setEditorTab, month, latitude } = useDsdStore();

  const GIF_DURATIONS = [3, 5, 10, 15, 30] as const;
  const GIF_DELAY_MS  = 80;

  const DOCK_TABS: { key: DsdEditorTab; icon: React.ReactNode; label: string }[] = [
    { key: 'shape',    icon: <CropSquareRoundedIcon sx={{ fontSize: 18 }} />, label: '形状' },
    { key: 'sun',      icon: <WbSunnyRoundedIcon   sx={{ fontSize: 18 }} />, label: '日照' },
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[SunDiagramEditor] Thumbnail upload failed (non-fatal)', thumbErr);
      }
      useDsdStore.getState().bumpSavedTick(); // 未保存ドット解除トリガ
      setSaveStatus('saved');
      setTimeout(() => setSaveStatus('idle'), 2500);
    } catch (e) {
      console.error('[SunDiagramEditor] Save failed', e);
      setSaveStatus('error');
  ...
  const handleExportGif = useCallback(async (w: number, h: number) => {
    ...
    try {
      ...
      for (let i = 0; i < FRAMES; i++) {
        ...
        const imageData = handle.renderFrame(w, h, hour);
  ...
  }, [diagramTitle, latitude, month, gifDurationSec, GIF_DELAY_MS]);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default', overflow: 'hidden' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ position: 'relative' }}>
          <Button
            size="small"
            variant="contained"
            startIcon={isGeneratingGif ? <CircularProgress size={12} sx={{ color: '#1b2a00' }} /> : <DownloadRoundedIcon fontSize="small" />}
            onClick={() => !isGeneratingGif && setExportMenuOpen(!exportMenuOpen)}
            disabled={isGeneratingGif}
            sx={{ bgcolor: '#aed581', color: '#1b2a00', '&:hover': { bgcolor: '#c5e1a5' }, fontSize: '0.75rem', fontWeight: 700, textTransform: 'none', '&.Mui-disabled': { bgcolor: 'rgba(174,213,129,0.4)', color: '#1b2a00' } }}
          >
      ...
      >
        ...
        {/* ── Floating dock — left edge of canvas ── */}
        <Box sx={{
