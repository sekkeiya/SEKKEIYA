...
import { useDsdStore, type SiteEditorTab } from './store/useDsdStore';
import { SiteDiagramCanvas, type SiteDiagramCanvasHandle } from './canvas/SiteDiagramCanvas';
...
const DOCK_TABS: { key: SiteEditorTab; icon: React.ReactNode; label: string }[] = [
  { key: 'site',     icon: <MapRoundedIcon        sx={{ fontSize: 18 }} />, label: '敷地設定' },
  { key: 'context',  icon: <LayersRoundedIcon      sx={{ fontSize: 18 }} />, label: 'コンテキスト' },
  { key: 'access',   icon: <DirectionsRoundedIcon  sx={{ fontSize: 18 }} />, label: 'アクセス' },
  { key: 'style',    icon: <PaletteRoundedIcon     sx={{ fontSize: 18 }} />, label: 'スタイル' },
  { key: 'annotate', icon: <EditRoundedIcon        sx={{ fontSize: 18 }} />, label: '注記' },
];

export const SiteDiagramEditor: React.FC = () => {
  ...
  const canvasRef = useRef<SiteDiagramCanvasHandle>(null);
  ...
  const { style, setStyle, siteEditorTab, setSiteEditorTab, siteAccesses } = useDsdStore();

  const diagramTitle = '敷地・アクセスダイアグラム';
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[SiteDiagramEditor] Thumbnail upload failed (non-fatal)', thumbErr);
      }
      useDsdStore.getState().bumpSavedTick(); // 未保存ドット解除トリガ
      setSaveStatus('saved');
      setTimeout(() => setSaveStatus('idle'), 2500);
    } catch (e) {
      console.error('[SiteDiagramEditor] Save failed', e);
      setSaveStatus('error');
  ...
  const handleExportGif = useCallback(async (w: number, h: number) => {
    ...
    try {
      ...
      try {
        ...
        const a = document.createElement('a');
        a.href = url; a.download = `${diagramTitle}_${gifDurationSec}s_${w}x${h}.gif`;
  ...
  const gifReady = siteAccesses.length > 0;
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default', overflow: 'hidden' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ position: 'relative' }}>
          <Button
            size="small"
            variant="contained"
            startIcon={isGeneratingGif ? <CircularProgress size={12} sx={{ color: '#003d47' }} /> : <DownloadRoundedIcon fontSize="small" />}
            onClick={() => !isGeneratingGif && setExportMenuOpen(!exportMenuOpen)}
            disabled={isGeneratingGif}
            sx={{
              bgcolor: ACCENT, color: '#003d47',
              '&:hover': { bgcolor: '#80deea' },
              '&.Mui-disabled': { bgcolor: 'rgba(77,208,225,0.4)', color: '#003d47' },
              ...
              {!gifReady && (
                <Box sx={{ px: 2, py: 0.5 }}>
                  <Typography variant="caption" sx={{ color: BRAND.sub2, fontSize: '0.66rem', lineHeight: 1.4 }}>
                    アクセスを追加するとドット移動 GIF になります
                  </Typography>
                </Box>
              )}
              {exportError && (
                <Box sx={{ px: 2, py: 0.5 }}>
                  <Typography variant="caption" sx={{ color: '#ef5350', fontSize: '0.66rem' }}>{exportError}</Typography>
                </Box>
              )}
      ...
      >
        <SiteDiagramCanvas ref={canvasRef} width={canvasSize.w} height={canvasSize.h} />
        ...
        <Box sx={{
          ...
          {DOCK_TABS.map(t => (
            <Tooltip key={t.key} title={t.label} placement="right">
              <Box
                onClick={() => setSiteEditorTab(t.key)}
                sx={{
                  width: 32, height: 32,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
