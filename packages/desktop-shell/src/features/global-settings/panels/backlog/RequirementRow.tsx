...
import { keyOf, COL_COUNT, type Platform } from './rowConstants';
import {
...
export interface RequirementRowProps {
  ...
  fixCollapsed: boolean;             // この要件の修正チェックリストを畳んでいるか
  ...
  return (
    <>
      <TableRow hover sx={{ opacity: done ? 0.6 : 1, '& td': { py: 0.25 }, '& .row-del': { opacity: 0, transition: 'opacity .12s' }, '&:hover .row-del': { opacity: 1 } }}>
        ...
        {/* 要件9: 理由列 */}
        <TableCell><InlineText value={item.reason} placeholder="理由…" onCommit={(v) => onPatch(item.id, { reason: v })} /></TableCell>
        ...
        <AttachCell item={item} onOpen={onOpenAttach} />
        {/* 要件62: テスト結果（手動テストの合否/所見。notes とは別列） */}
        <TableCell><InlineText value={item.testResult} placeholder="テスト結果…" onCommit={(v) => onPatch(item.id, { testResult: v })} /></TableCell>
        <TableCell padding="none" align="center">
