...
export const DsfRightPanel: React.FC<DsfRightPanelProps> = ({ item, onOpen, onSetVisibility }) => {
  ...
  return (
    <Box sx={{ p: 2.5, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Typography sx={{ fontSize: 14, fontWeight: 700, color: 'var(--brand-fg)', mb: 1.5 }}>ポートフォリオ情報</Typography>

      {/* Cover preview */}
      <Box sx={{ aspectRatio: '3 / 4', borderRadius: 1.5, overflow: 'hidden', bgcolor: 'light-dark(rgba(15,23,42,0.1), rgba(0,0,0,0.3))', mb: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', borderLeft: `3px solid ${ACCENT}` }}>
        {thumb ? (
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
        <Button
          ...
          onClick={() => onOpen?.(item)}
          sx={{ bgcolor: ACCENT, color: 'var(--brand-fg)', '&:hover': { bgcolor: '#9575cd' } }}
        >
          本を開く
        </Button>
        {item.downloadUrl && (
