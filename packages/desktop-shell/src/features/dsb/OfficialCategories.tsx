...
export const OfficialCategories: React.FC = () => {
  ...
  const renderRow = (c: BlogCategory, isChild: boolean) => (
    <Box key={c.id} onClick={() => openEdit(c)} sx={{
      ...
      bgcolor: editCat?.id === c.id && dialogOpen ? `${ACCENT}14` : (isChild ? 'rgba(255,255,255,0.015)' : 'transparent'),
      '&:hover': { bgcolor: 'rgba(255,255,255,0.03)' },
    }}>
      {isChild
        ? <SubdirectoryArrowRightRoundedIcon sx={{ fontSize: 16, color: 'rgba(255,255,255,0.25)' }} />
        : <Typography sx={{ width: 22, color: 'rgba(255,255,255,0.3)', fontFamily: 'monospace', fontSize: '0.78rem' }}>{c.order ?? '-'}</Typography>}
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
          <Typography sx={{ color: '#fff', fontWeight: isChild ? 600 : 800, fontSize: isChild ? '0.9rem' : '1rem' }}>{c.name}</Typography>
          ...
          {c.active === false && <Chip label="無効" size="small" sx={{ height: 18, fontSize: '0.62rem', bgcolor: 'rgba(107,114,128,0.2)', color: 'rgba(255,255,255,0.5)' }} />}
        </Stack>
        {c.description && <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', mt: 0.25 }}>{c.description}</Typography>}
      </Box>
      {!isChild && (
        <Button size="small" startIcon={<AddRoundedIcon sx={{ fontSize: 14 }} />} onClick={(e) => { e.stopPropagation(); openAdd(c.slug); }}
          sx={{ color: 'rgba(255,255,255,0.5)', textTransform: 'none', fontSize: '0.72rem', minWidth: 0 }}>サブ追加</Button>
      )}
      <FormControlLabel onClick={(e) => e.stopPropagation()}
        ...
        label={<Typography sx={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.5)' }}>有効</Typography>} sx={{ mr: 0 }} />
  ...
  return (
    <Box sx={{ flex: 1, height: '100%', overflowY: 'auto', bgcolor: 'background.default' }}>
      <Box sx={{ maxWidth: 1120, mx: 'auto', width: '100%', p: { xs: 2.5, md: 4 } }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 800, color: '#fff' }}>カテゴリ管理</Typography>
          </Box>
        ...
        {loading ? (
          ...
          <Box sx={{ p: 5, borderRadius: 2, border: `1px dashed ${ACCENT}4d`, textAlign: 'center' }}>
            <Typography sx={{ color: 'rgba(255,255,255,0.6)', mb: 1 }}>カテゴリがまだありません</Typography>
      ...
      <Drawer anchor="right" open={dialogOpen} onClose={() => setDialogOpen(false)}
        PaperProps={{ sx: { width: { xs: '100%', sm: 440 }, bgcolor: '#0e121c', borderLeft: `1px solid ${BRAND.line}`, color: '#fff' } }}>
        <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${BRAND.line}` }}>
          <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.05rem' }}>{editCat ? 'カテゴリを編集' : 'カテゴリを追加'}</Typography>
        ...
        <Box sx={{ p: 2.5, flex: 1, overflowY: 'auto' }}>
          <Stack spacing={2.5}>
            <TextField label="カテゴリ名 *" fullWidth size="small" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              placeholder="例: AI × 空間設計" InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }}
            ...
            <TextField label="スラッグ（空欄なら自動生成）" fullWidth size="small" value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))}
              placeholder="例: ai-design" InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }}
              sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }} />
            ...
            <TextField select label="親カテゴリ（サブにする場合）" fullWidth size="small" value={form.parent} onChange={(e) => setForm((f) => ({ ...f, parent: e.target.value }))}
              helperText="なし＝トップレベル（公開一覧のフィルタに出る）" InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }}
            ...
            <TextField label="並び順" type="number" size="small" value={form.order} onChange={(e) => setForm((f) => ({ ...f, order: e.target.value }))}
              sx={{ width: 120, '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: 'rgba(255,255,255,0.15)' } } }}
              InputLabelProps={{ sx: { color: 'rgba(255,255,255,0.5)' } }} />
            <FormControlLabel control={<Switch checked={form.active} onChange={(e) => setForm((f) => ({ ...f, active: e.target.checked }))} />}
              label={<Typography sx={{ color: 'rgba(255,255,255,0.7)' }}>有効（トピック・ネタ提案・公開フィルタに表示）</Typography>} />
            {editCat && (
      ...
      <Drawer anchor="bottom" open={!!confirm} onClose={() => setConfirm(null)}
        ...
        {confirm && (
          <Box sx={{ maxWidth: 460, mx: 'auto', mb: 3, p: 2.5, bgcolor: '#0e121c', border: `1px solid ${BRAND.line}`, borderRadius: 3 }}>
            <Typography sx={{ color: '#fff', fontWeight: 800, mb: 1 }}>カテゴリを削除</Typography>
            ...
            <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'flex-end' }}>
              ...
              <Button onClick={() => void doDelete(confirm)} variant="contained" sx={{ bgcolor: '#ef4444', color: '#fff', fontWeight: 800, '&:hover': { bgcolor: '#dc2626' } }}>削除する</Button>
            </Box>
          </Box>
        )}
      </Drawer>
