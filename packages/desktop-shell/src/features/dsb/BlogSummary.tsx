    ...
    <Box sx={{
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        <Box sx={{ width: 24, height: 24, borderRadius: 1.5, bgcolor: color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {React.cloneElement(icon as React.ReactElement<any>, { sx: { fontSize: 15, color: '#fff' } })}
        </Box>
        <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.55)' }}>{label}</Typography>
      </Box>
      <Typography sx={{ fontSize: 26, fontWeight: 700, color: '#fff', lineHeight: 1 }}>{value}</Typography>
    </Box>
  );
}
    ...
    <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', px: { xs: 2, md: 4 }, py: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: 20, lineHeight: 1.2 }}>ブログ</Typography>
          <Typography sx={{ color: 'rgba(255,255,255,0.45)', fontSize: 12 }}>{scopeLabel} の記事の状況</Typography>
        </Box>
        <Button
        ...
        <>
          ...
          {scoped.length === 0 ? (
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, alignItems: 'flex-start' }}>
              {/* 閲覧数ランキング */}
              <Box sx={{ flex: '1 1 420px', minWidth: 300, p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                  <VisibilityRoundedIcon sx={{ fontSize: 16, color: ACCENT }} />
                  <Typography sx={{ color: '#fff', fontSize: 13, fontWeight: 700 }}>記事別の閲覧数（公開記事）</Typography>
                </Box>
                {ranking.items.length === 0 ? (
                  <Typography sx={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', py: 2 }}>公開記事がありません。記事を公開すると閲覧数が並びます。</Typography>
                ) : (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {ranking.items.map((a: BlogArticle, i) => {
                      ...
                      return (
                        <Box key={a.id} onClick={() => startEdit(a.id)} sx={{ cursor: 'pointer', '&:hover .ttl': { color: '#fff' } }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.4 }}>
                            <Typography sx={{ fontSize: 11, fontWeight: 800, color: 'rgba(255,255,255,0.3)', width: 16 }}>{i + 1}</Typography>
                            <Typography noWrap className="ttl" sx={{ flex: 1, minWidth: 0, fontSize: 12.5, color: 'rgba(255,255,255,0.8)' }}>{a.title || '(無題)'}</Typography>
                            <Typography sx={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>{typeof a.views === 'number' ? v.toLocaleString() : '—'}</Typography>
                          </Box>
                          <Box sx={{ height: 5, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.06)', overflow: 'hidden', ml: 3 }}>
                    ...
                    {!stats.anyViews && (
                      <Typography sx={{ fontSize: 10.5, color: 'rgba(255,255,255,0.3)', mt: 0.5 }}>
              ...
              {/* カテゴリ別の記事数 */}
              <Box sx={{ flex: '1 1 320px', minWidth: 280, p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <Typography sx={{ color: '#fff', fontSize: 13, fontWeight: 700, mb: 1.5 }}>カテゴリ別の記事数</Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {byCategory.rows.map((r) => (
                    <Box key={r.name}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.4 }}>
                        <Typography noWrap sx={{ flex: 1, minWidth: 0, fontSize: 12.5, color: 'rgba(255,255,255,0.8)' }}>{r.name}</Typography>
                        <Typography sx={{ fontSize: 11, color: 'rgba(255,255,255,0.45)' }}>公開{r.published}・下書き{r.draft}</Typography>
                        <Typography sx={{ fontSize: 12, fontWeight: 700, color: '#fff', width: 24, textAlign: 'right' }}>{r.total}</Typography>
                      </Box>
                      <Box sx={{ display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', bgcolor: 'rgba(255,255,255,0.06)' }}>
                        ...
                        <Box sx={{ width: `${(r.draft / byCategory.max) * 100}%`, bgcolor: 'rgba(255,255,255,0.25)' }} />
                      </Box>
                    </Box>
                  ))}
                </Box>
              ...
              {/* 今後の投稿予定（スケジュールへの導線） */}
              <Box sx={{ flex: '1 1 100%', p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                  <EventNoteRoundedIcon sx={{ fontSize: 16, color: ACCENT }} />
                  <Typography sx={{ color: '#fff', fontSize: 13, fontWeight: 700, flex: 1 }}>今後の投稿予定</Typography>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                    {upcoming.map((s) => (
                      <Box key={s.id} onClick={() => setView('schedule')}
                        sx={{ cursor: 'pointer', flex: '1 1 200px', minWidth: 180, px: 1.5, py: 1, borderRadius: 1.5, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', '&:hover': { borderColor: `${ACCENT}66` } }}>
                        <Typography sx={{ fontSize: 11, fontWeight: 700, color: ACCENT }}>{s.date}</Typography>
                        <Typography noWrap sx={{ fontSize: 12.5, color: '#fff', fontWeight: 600 }}>{s.title}</Typography>
                        {s.category && <Chip size="small" label={s.category} sx={{ mt: 0.5, height: 18, fontSize: 9.5, bgcolor: `${ACCENT}26`, color: '#f3c0c0' }} />}
                      </Box>
                    ))}
                  </Box>
                )}
