...
import { categorizeByVision } from '../utils/visionCategorize';
...
export const useUploadHandlers = ({
  ...
  const handleFilesDrop = async (eOrFiles) => {
    ...
    // .gh files must attach to a .3dm item — they cannot be saved standalone
    const ghFiles = validModelFiles.filter(f => f.name.toLowerCase().endsWith('.gh'));
    const nonGhModelFiles = validModelFiles.filter(f => !f.name.toLowerCase().endsWith('.gh'));

    // If ONLY .gh files were dropped, attach to existing .3dm queue item or add as standalone
    if (nonGhModelFiles.length === 0 && ghFiles.length > 0) {
      const existing3dm = uploadQueue?.find(
        item => item.ext === '3dm' && ['queued', 'ready', 'parsing', 'error'].includes(item.status)
      );
      if (existing3dm) {
        setters.updateQueueItem(existing3dm.id, { ghFiles: [...(existing3dm.ghFiles || []), ...ghFiles] });
        if (setIsLoadingFiles) setIsLoadingFiles(false);
        return;
      }
      // No .3dm in queue — add .gh items as standalone (user selects Rhino in the queue card)
      const ghItems = ghFiles.map(f => {
        const item = createQueueItem(f);
        item.uploadEnabled = false;
        item.status = 'queued'; // skip 'parsing' state — no async processing needed for standalone .gh
        return item;
      });
      setters.appendFilesToQueue(ghItems);
      if (setIsLoadingFiles) setIsLoadingFiles(false);
      return;
    }
    ...
    const nonSupportedCount = uniqueFiles.length - validModelFiles.length;
    if (nonSupportedCount > 0) {
      skipMsg += `${nonSupportedCount}件の非対応ファイルをスキップしました。\n`;
    }

    const addedCount = nonGhModelFiles.length;
    const ghAttachMsg = ghFiles.length > 0 ? `（GHスクリプト ${ghFiles.length}件を.3dmに添付）` : '';
    if (skipMsg) {
      alert(`${skipMsg}\n${addedCount}件のファイルをキューに追加しました。${ghAttachMsg}`);
    }

    if (setLoadingTotal) setLoadingTotal(addedCount);
    if (setLoadingMessage) setLoadingMessage("ファイルを解析しています...");

    const newItems = [];
    for (let i = 0; i < addedCount; i++) {
        newItems.push(createQueueItem(nonGhModelFiles[i]));

        // Update progress per item
        if (setLoadingCurrent) setLoadingCurrent(i + 1);
        if (setLoadingProgress) setLoadingProgress(Math.round(((i + 1) / addedCount) * 100));

        // Yield every few files to allow the UI progress bar to animate smoothly without freezing
        if (i % 5 === 0) await new Promise(r => setTimeout(r, 0));
    }

    // Attach .gh files to first .3dm in the new batch, or fall back to an existing queue item
    if (ghFiles.length > 0) {
      const target3dm = newItems.find(item => item.ext === '3dm');
      if (target3dm) {
        target3dm.ghFiles = ghFiles;
      } else {
        const existing3dm = uploadQueue?.find(
          item => item.ext === '3dm' && ['queued', 'ready', 'parsing', 'error'].includes(item.status)
        );
        if (existing3dm) {
          ...
          // No .3dm anywhere — add .gh as standalone items (user selects Rhino in the queue card)
          for (const ghFile of ghFiles) {
            const ghItem = createQueueItem(ghFile);
            ghItem.uploadEnabled = false;
            ghItem.status = 'queued'; // skip 'parsing' state
            newItems.push(ghItem);
          }
        }
      }
    }
    ...
    try {
      ...
      console.warn("Failed to fetch existing models for similarity search", err);
        ...
        for (const item of newItems) {
            // .gh standalone items need no processing (no GLB conversion, no thumbnail)
            if (item.ext === 'gh') continue;
            await processModelItem(item, existingDocs);
        }
  ...
  // Process a single model item: lightweight metadata, GLB conversion, thumbnail,
  // dimension extraction, and hybrid (rule + vision AI) category/tag auto-fill.
  // Shared by handleFilesDrop and the late Rhino-attach flow (attachRhinoToGhItem).
  const processModelItem = async (item, existingDocs = []) => {
            ...
            setters.updateQueueItem(item.id, {
                ...
                _autoTitle: autoFilled.title, // AI補完で上書きしてよいかの比較基準
                ...
                _autoMain: autoFilled.mainCategory || "", // 後段でユーザー手動編集と区別するため
                status: 'queued',
        ...
        let aiImageBlob = null; // vision AI に送るサムネイル(あれば)
        ...
        if (ext === 'glb') {
            ...
            let glbFile = null;
                try {
                    ...
                    return; // Abort processing this item if GLB generation failed
                }
        ...
        // 型はマクロカテゴリ名 (例: '家具 (既製品)' / '建築・空間') で扱う。
        // ルール推定で確定していればそれを使い、無ければ類似モデルのスコアで補完。
        let fallbackType = autoFilled.macroCategory || item.macroCategory || "";
        const { furnitureScore = 0, buildingScore = 0 } = similarityCategories;
        if (!autoFilled.macroCategory && (furnitureScore > 0 || buildingScore > 0)) {
           fallbackType = furnitureScore >= buildingScore ? "家具 (既製品)" : "建築・空間";
        }
        ...
        // --- 5. Vision AI 分類 → ルール結果と統合して最良を採用 ---
        //   サムネイルがあれば LLM(vision)へ送り、構造化カテゴリ/マテリアル/タグを取得。
        //   Function 未デプロイ・失敗時は null が返り、ルールベースのまま進む。
        let aiCat = null;        // 正規化済みの AI カテゴリ階層
        let aiMaterials = [];
        let aiTags = [];
        ...
        if (aiImageBlob) {
            try {
                const vision = await categorizeByVision({
                    thumbnailBlob: aiImageBlob,
                    title: normalizedFallback.title || autoFilled.title || item.title,
                    ruleTags: Array.from(newTagsSet),
                    dimensionsMm: extractedDims,
                    mergedCategoryMap,
                    timeoutMs: 30000,
                });
                if (vision) {
                    const normalizedAi = normalizeAndSanitizeCategory({
                        type: vision.macroCategory,
                        mainCategory: vision.mainCategory,
                        subCategory: vision.subCategory,
                        tags: vision.tags || [],
                    }, {}, mergedCategoryMap);
                    aiMaterials = Array.isArray(vision.materials) ? vision.materials : [];
                    aiTags = Array.isArray(vision.tags) ? vision.tags : [];
                    if (vision.suggestedTitle) aiSuggestedTitle = vision.suggestedTitle;
                    // AI が有効カテゴリを返し、ルールが空 or AI の確信度が十分なら AI を採用
                    const conf = typeof vision.confidence === 'number' ? vision.confidence : 1;
                    if (normalizedAi.mainCategory && (!normalizedFallback.mainCategory || conf >= 0.5)) {
                        aiCat = normalizedAi;
                        aiContributed = true;
                    }
                }
            } catch (e) {
                console.warn('[Upload] vision categorize failed, using rules', e);
            }
        }

        // 統合: AI採用時は AI の階層、無ければルール。sub は同一 main ならルール側を温存。
        const mergedMacro = aiContributed ? (aiCat.macroCategory || normalizedFallback.macroCategory) : normalizedFallback.macroCategory;
        const mergedMain  = aiContributed ? aiCat.mainCategory : normalizedFallback.mainCategory;
        let mergedSub = aiContributed ? aiCat.subCategory : normalizedFallback.subCategory;
        if (aiContributed && !mergedSub && normalizedFallback.mainCategory === mergedMain) {
            mergedSub = normalizedFallback.subCategory;
        }
        const mergedMaterials = Array.from(new Set([...(spatialInference.materials || []), ...aiMaterials]));
        const mergedTagsBase = Array.from(new Set([
            ...Array.from(newTagsSet),
            ...(spatialInference.newTags || []),
            ...aiTags,
        ])).slice(0, 12);

        setters.updateQueueItem(item.id, (currentItem) => {
           // ユーザーが手動で main を変更していれば尊重 (自動補完値 _autoMain と比較)
           const userEditedMain = currentItem.mainCategory !== (currentItem._autoMain ?? "");
           const finalMacro = userEditedMain ? currentItem.macroCategory : (mergedMacro || "家具 (既製品)");
           const finalMain  = userEditedMain ? currentItem.mainCategory : (mergedMain || "");
           const finalSub   = userEditedMain ? currentItem.subCategory : (mergedSub || "");
           // タグ/マテリアルは「失わない」ことを優先し和集合 (現在値 ∪ ルール ∪ AI)
           const tagsToUse = Array.from(new Set([...(currentItem.tags || []), ...mergedTagsBase])).slice(0, 12);
           const materialsToUse = Array.from(new Set([...(currentItem.materials || []), ...mergedMaterials])).slice(0, 6);
           const isAiSource = !!currentItem.ai?.prompt || !!item.file?.aiGenerated;
           const usedAi = aiContributed && !userEditedMain;
           // ユーザーがタイトルを手動編集していなければ AI の提案タイトルで上書き
           const userEditedTitle = currentItem.title !== (currentItem._autoTitle ?? currentItem.title);
           const finalTitle = (!userEditedTitle && aiSuggestedTitle) ? aiSuggestedTitle : currentItem.title;
           // バッジ: vision AI採用 or 生成元AI → 「🪄 AI補完」/ ルールのみ → 「✨ ルール自動入力」
           return {
               aiStatus: 'done',
               title: finalTitle,
               _autoTitle: finalTitle, // 次回 AI 補完時の比較基準を最新に保つ
               macroCategory: finalMacro,
               ...
               ruleApplied: { type: !!finalMain, mainCategory: !!finalMain, subCategory: !!finalSub, tags: (tagsToUse?.length || 0) > 0 },
               ai: { generated: usedAi || isAiSource, source: usedAi ? 'vision-ai' : 'hybrid-local', prompt: currentItem.ai?.prompt || '' }
           };
        });
  };

  // Late attach: user dropped a standalone .gh, then picked a Rhino (.3dm) to bind it to.
  // Transform the .gh queue item into a .3dm primary (with the .gh attached) and run the
  // same auto-fill pipeline a fresh .3dm drop would (GLB conversion, thumbnail, category/tags).
  const attachRhinoToGhItem = async (ghItem, rhinoFile) => {
    if (!ghItem || !rhinoFile) return;
    const combinedGhFiles = [ghItem.file, ...(ghItem.ghFiles || [])].filter(Boolean);

    // Transform queue item into a .3dm primary with the .gh attached
    setters.updateQueueItem(ghItem.id, {
      file: rhinoFile,
      filename: rhinoFile.name,
      ext: '3dm',
      title: rhinoFile.name,
      ghFiles: combinedGhFiles,
      uploadEnabled: true,
      status: 'parsing',
      thumbnailPreviewUrl: '',
      thumbnailBlob: null,
      dimensions: { width: '', depth: '', height: '', unit: 'mm', autoExtracted: false, extractError: null },
      dimensionSource: 'none',
    });

    // Fetch recent models once for local similarity inference (mirror handleFilesDrop)
    let existingDocs = [];
    try {
      if (user?.uid) {
        const q = query(
        ...
        const snapshot = await getDocs(q);
        existingDocs = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      }
    } catch (err) {
      console.warn("Failed to fetch existing models for similarity search", err);
    }

    // Snapshot reflecting the new primary file for processing
    const itemToProcess = {
      ...ghItem,
      file: rhinoFile,
      filename: rhinoFile.name,
    ...
    try {
      await processModelItem(itemToProcess, existingDocs);
    } catch (err) {
      console.error("Failed to process attached Rhino file:", err);
      setters.updateQueueItem(ghItem.id, { status: 'error', errorMsg: err.message || "処理に失敗しました" });
    }
  };
  ...
  const handleProcessQueue = async (uploadQueue) => {
    ...
    for (const item of itemsToProcess) {
      try {
        ...
        // 2.6) Grasshopper files upload (only for .3dm items)
        let uploadedGhFiles = [];
        if (item.ext === '3dm' && item.ghFiles && item.ghFiles.length > 0) {
            for (const ghFile of item.ghFiles) {
                const ghPath = `${storageDir}/gh/${ghFile.name}`;
                const ghRef = ref(storage, ghPath);
                const ghTask = uploadBytesResumable(ghRef, ghFile, uploadMetadata);
                await new Promise((resolve, reject) => ghTask.on('state_changed', null, reject, resolve));
                const ghUrl = await getDownloadURL(ghRef);
                uploadedGhFiles.push({ name: ghFile.name, downloadUrl: ghUrl, storagePath: ghPath, size: ghFile.size });
            }
        }
        ...
        // Save AI taxonomy, Companion GLB, and GH files data
        const extendedMetadata = {
           ...
           ghFiles: uploadedGhFiles.length > 0 ? uploadedGhFiles : null,
        };
