...
const UploadModalContent = React.forwardRef(({ open, onClose, initialFiles, rhinoJob }, ref) => {
  ...
  const seedDimensions = useDssUploadBridge(s => s.seedDimensions);
  const latestQueueRef = React.useRef([]);
  latestQueueRef.current = uploadQueue;
  ...
  React.useEffect(() => {
    ...
    (async () => {
      // 商品ページ由来の実寸(mm)があれば、AI推定より優先して各アイテムの W/D/H に反映してから保存。
      const sd = useDssUploadBridge.getState().seedDimensions;
      if (sd && (sd.width || sd.depth || sd.height)) {
        latestQueueRef.current.forEach((it) => {
          const s = it.size || {};
          setters.updateQueueItem(it.id, { size: {
            ...s,
            width: sd.width != null ? String(sd.width) : s.width,
            depth: sd.depth != null ? String(sd.depth) : s.depth,
            height: sd.height != null ? String(sd.height) : s.height,
          } });
        });
        await new Promise((r) => setTimeout(r, 350)); // state反映待ち（更新後のキューで保存）
      }
      await handleProcessQueue(latestQueueRef.current);
      setToastMsg('追加完了しました');
      setToastOpen(true);
      setTimeout(() => { onClose(); }, 1500);
    })();
  }, [open, autoSaveToModels, canUpload, initialFiles, seedDimensions]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          <Box sx={{ 
            ...
            {filteredQueue.map((item) => (
              <UploadQueueItemCard
                ...
                item={item}
                setters={setters}
                onAttachRhino={attachRhinoToGhItem}
