...
export const normalizeAndSanitizeCategory = (data, similarityScores = {}, mergedCategoryMap = null) => {
  ...
  try {
    ...
    // ゆるい型ラベル(家具/建築/Furniture/Architecture)を実際のマクロキーへ解決する。
    // 例: "家具" → "家具 (既製品)" / "建築" → "建築・空間"
    const resolveMacro = (t) => {
      if (!t) return null;
      if (allowedTypes.includes(t)) return t;
      const s = String(t);
      if (s.includes('家具') || s === 'Furniture') return allowedTypes.find(k => k.includes('家具')) || null;
      if (s.includes('建築') || s === 'Architecture' || s === '建物') return allowedTypes.find(k => k.includes('建築')) || null;
      if (s.includes('設備')) return allowedTypes.find(k => k.includes('設備')) || null;
      if (s.includes('グリーン')) return allowedTypes.find(k => k.includes('グリーン')) || null;
      if (s.includes('インテリア')) return allowedTypes.find(k => k.includes('インテリア')) || null;
      return null;
    };

    const resolvedType = resolveMacro(data.type);
    if (resolvedType) {
    ...
    if (result.type === "その他" || !result.type) {
      ...
      const resolvedOverride = resolveMacro(overrideType);
      if (resolvedOverride) {
        console.log(`[Type Auto-Correction] Overrode 'その他'/missing to '${resolvedOverride}' based on ${reason}.`);
        result.type = resolvedOverride;
      } else {
  ...
  - no furniture/building keyword`);
    ...
    // macroCategory は type と同義 (呼び出し側が参照する)
    result.macroCategory = result.type;
    return result;
