...
export const LensResultsDialog: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2.5, py: 1.5, borderBottom: '1px solid rgb(var(--brand-fg-rgb) / 0.08)' }}>
        <ImageSearchRoundedIcon sx={{ fontSize: 20, color: 'light-dark(#0352aa, #93c5fd)' }} />
        ...
        {lensUrl && (
          <Button
            ...
            sx={{ color: 'light-dark(#0352aa, #93c5fd)', fontSize: 12, textTransform: 'none', mr: 0.5 }}
          >
            ブラウザで開く
          </Button>
        )}
      ...
      <Box sx={{ p: 2.5 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
          {queryImage && (
            <Box sx={{ width: 64, height: 48, borderRadius: 1, overflow: 'hidden', flexShrink: 0, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.05)' }}>
              <img src={queryImage} alt="query" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </Box>
          )}
          <Box sx={{ minWidth: 0 }}>
            <Typography sx={{ fontSize: 11, color: 'rgb(var(--slate-ink-rgb) / 0.9)' }}>このモデルに似た商品リンク</Typography>
        ...
        {busy && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 5, justifyContent: 'center' }}>
            ...
            <Typography sx={{ fontSize: 13, color: 'light-dark(rgba(31,41,55,0.85), rgba(229,231,235,0.85))' }}>Google レンズで検索中…（初回はモデル読込に時間がかかります）</Typography>
          </Box>
        )}
        ...
        {!busy && !error && results.length === 0 && (
          <Box sx={{ py: 4, textAlign: 'center' }}>
            <Typography sx={{ fontSize: 13, color: 'light-dark(rgba(31,41,55,0.8), rgba(229,231,235,0.8))' }}>検索結果を取得できませんでした。</Typography>
              ...
              return (
                <Box
                  ...
                  sx={{
                    ...
                    border: isSel ? '1.5px solid #60a5fa' : '1px solid rgb(var(--brand-fg-rgb) / 0.08)',
                    bgcolor: 'rgb(var(--brand-fg-rgb) / 0.03)', cursor: 'pointer', transition: 'border-color 0.15s',
                ...
                >
                  <Checkbox
                    checked={isSel}
                    size="small"
                    sx={{ position: 'absolute', top: 2, left: 2, zIndex: 2, color: 'rgb(var(--brand-fg-rgb) / 0.7)', '&.Mui-checked': { color: 'light-dark(#054ea8, #60a5fa)' }, bgcolor: 'light-dark(rgba(15,23,42,0.12), rgba(0,0,0,0.35))', p: 0.25, borderRadius: 1 }}
                  />
                  <IconButton
                    size="small"
                    onClick={(e) => { e.stopPropagation(); openExternalUrl(r.url); }}
                    sx={{ position: 'absolute', top: 2, right: 2, zIndex: 2, color: 'var(--brand-fg)', bgcolor: 'rgba(0,0,0,0.35)', p: 0.5, '&:hover': { color: 'light-dark(#0352aa, #93c5fd)', bgcolor: 'rgba(0,0,0,0.55)' } }}
                  >
                    <OpenInNewRoundedIcon sx={{ fontSize: 14 }} />
                  </IconButton>
                  <Box sx={{ aspectRatio: '1/1', bgcolor: 'light-dark(rgba(15,23,42,0.1), rgba(0,0,0,0.3))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {r.thumbnail ? (
                      <img src={r.thumbnail} alt={r.title} style={{ width: '100%', height: '100%', objectFit: 'contain' }} referrerPolicy="no-referrer" />
                    ) : (
                      <LinkRoundedIcon sx={{ fontSize: 28, color: 'rgb(var(--slate-ink-rgb) / 0.6)' }} />
                    )}
                  </Box>
                  <Box sx={{ p: 1 }}>
                    <Typography noWrap sx={{ fontSize: 11.5, fontWeight: 600 }}>{r.title || '(タイトルなし)'}</Typography>
                    <Typography noWrap sx={{ fontSize: 10.5, color: 'rgb(var(--slate-ink-rgb) / 0.9)' }}>{r.source}</Typography>
                  </Box>
                </Box>
              );
            })}
        ...
        {!busy && showDiagSection && (
          <Box sx={{ mt: 2, pt: 1.5, borderTop: '1px solid rgb(var(--brand-fg-rgb) / 0.06)' }}>
            <Typography
              onClick={() => setShowDiag((v) => !v)}
              sx={{ fontSize: 11, color: 'rgb(var(--slate-ink-rgb) / 0.9)', cursor: 'pointer', userSelect: 'none', '&:hover': { color: 'light-dark(#0352aa, #93c5fd)' } }}
            >
            ...
            {showDiag && (
              <Box
                ...
                sx={{
                  ...
                  bgcolor: 'rgba(0,0,0,0.45)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.08)',
                  fontSize: 10.5, lineHeight: 1.5, color: 'light-dark(rgba(31,41,55,0.85), rgba(229,231,235,0.85))', whiteSpace: 'pre-wrap', wordBreak: 'break-all',
                }}
              >
 ...
 (diag.sample || []).map((s, i) =>
   `\n[${i}] ${s.href}\n` + (s.imgs || []).map((im) =>
     `  - ${im.t} rw=${im.rw ?? '?'} nw=${im.nw ?? '?'} alt="${im.alt || ''}" ${im.src}`
      ...
      {!busy && !error && results.length > 0 && (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2.5, py: 1.5, borderTop: '1px solid rgb(var(--brand-fg-rgb) / 0.08)' }}>
          <Typography sx={{ fontSize: 12, color: 'rgb(var(--slate-ink-rgb) / 0.9)' }}>{selected.size} 件選択中</Typography>
          {!canRegister && (
            <Typography sx={{ fontSize: 11, color: 'light-dark(rgba(170,78,3,0.9), rgba(251,146,60,0.9))' }}>※ 自分のモデルのみ登録できます</Typography>
          )}
          <Box sx={{ flex: 1 }} />
          <Button onClick={onClose} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', textTransform: 'none', fontSize: 13 }}>キャンセル</Button>
          <Button
            variant="outlined"
            ...
            sx={{ textTransform: 'none', fontSize: 13, color: 'light-dark(#0352aa, #93c5fd)', borderColor: 'rgba(96,165,250,0.5)', '&:hover': { borderColor: '#60a5fa', bgcolor: 'rgba(96,165,250,0.1)' } }}
          >
          ...
          <Button
            ...
            startIcon={registering ? <CircularProgress size={14} sx={{ color: 'var(--brand-fg)' }} /> : <LinkRoundedIcon sx={{ fontSize: 16 }} />}
