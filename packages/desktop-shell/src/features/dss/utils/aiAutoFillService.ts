...
import { isNoiseToken } from '../upload/utils/smartCategoryEngine';
...
export function ruleBasedClassify(title: string, currentTags: string[], categoryHint?: string): {
  ...
  // 既存タグはノイズ(Firebaseパス/ID等)を除いて引き継ぐ
  const suggestedTags = new Set((currentTags || []).filter(t => t && !isNoiseToken(String(t))));
  ...
  // 一致なし: 「未分類」のノイズを入れず空で返す (UIの詳細欄を散らかさない)
  return {
    newTags: Array.from(suggestedTags),
    materials: Array.from(suggestedMaterials),
    rooms: [],
