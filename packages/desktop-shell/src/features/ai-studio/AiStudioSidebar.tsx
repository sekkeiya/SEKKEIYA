          ...
          {!collapsed && (
            <>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography sx={{ fontSize: 14, fontWeight: 800, color: '#fff', lineHeight: 1.1 }}>AI Studio</Typography>
                <Typography sx={{ fontSize: 10.5, color: 'rgba(255,255,255,0.4)' }}>AI & RAG コントロール</Typography>
              </Box>
              {onToggleCollapsed && (
                <Tooltip title="サイドバーを畳む" placement="right">
                  <IconButton size="small" onClick={onToggleCollapsed} sx={{ color: 'rgba(255,255,255,0.45)', '&:hover': { color: '#fff', bgcolor: 'rgba(255,255,255,0.06)' } }}>
              ...
              <Tooltip key={item.id} title={item.label} placement="right">
                <ButtonBase
                  ...
                  sx={{
                    ...
                    color: active ? '#c4a3f7' : 'rgba(255,255,255,0.5)',
                    '&:hover': { bgcolor: active ? 'rgba(168,85,247,0.22)' : 'rgba(255,255,255,0.06)', color: active ? '#c4a3f7' : '#fff' },
                  }}
                >
                  {item.icon}
                </ButtonBase>
        ...
        <Box sx={{ flex: 1, px: 1.5 }}>
          {SECTIONS.map((section, si) => (
            <Box key={si} sx={{ mb: 1.5 }}>
              {section.heading && (
                <Typography sx={{ px: 1.5, mt: 1.5, mb: 0.75, fontSize: 10, fontWeight: 700, letterSpacing: 1, color: 'rgba(255,255,255,0.32)', textTransform: 'uppercase' }}>
                  {section.heading}
                </Typography>
              )}
                ...
                return (
                  <ButtonBase
                    ...
                    sx={{
                      ...
                      '&:hover': { bgcolor: active ? 'rgba(168,85,247,0.2)' : 'rgba(255,255,255,0.05)' },
                    }}
                  >
                    <Box sx={{ color: active ? '#c4a3f7' : 'rgba(255,255,255,0.5)', display: 'flex', mt: '1px' }}>{item.icon}</Box>
                    <Box sx={{ minWidth: 0, flex: 1 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? '#fff' : 'rgba(255,255,255,0.78)', lineHeight: 1.2 }}>{item.label}</Typography>
                      <Typography sx={{ fontSize: 10.5, color: 'rgba(255,255,255,0.4)', lineHeight: 1.2, mt: 0.25, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.desc}</Typography>
                    </Box>
                  </ButtonBase>
                );
              })}
      ...
      <Box sx={{ p: collapsed ? 1 : 1.5, borderTop: `1px solid ${BRAND.line}`, display: 'flex', justifyContent: 'center' }}>
        {collapsed ? (
          <Tooltip title={`使用中のAI: ${activeProfile ? activeProfile.name : '未選択'}`} placement="right">
            <ButtonBase onClick={() => onViewChange('aimodels')} sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', '&:hover': { bgcolor: 'rgba(255,255,255,0.06)' } }}>
              <FiberManualRecordIcon sx={{ fontSize: 12, color: activeProfile ? '#4ade80' : 'rgba(255,255,255,0.3)' }} />
            </ButtonBase>
          </Tooltip>
        ) : (
          <ButtonBase
            onClick={() => onViewChange('aimodels')}
            sx={{ width: '100%', justifyContent: 'flex-start', gap: 1, p: 1.25, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', '&:hover': { bgcolor: 'rgba(255,255,255,0.06)' } }}
          >
            <FiberManualRecordIcon sx={{ fontSize: 10, color: activeProfile ? '#4ade80' : 'rgba(255,255,255,0.3)' }} />
            <Box sx={{ minWidth: 0, flex: 1, textAlign: 'left' }}>
              <Typography sx={{ fontSize: 9.5, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 0.5 }}>使用中のAI</Typography>
              <Typography sx={{ fontSize: 12, fontWeight: 600, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activeProfile ? activeProfile.name : '未選択'}</Typography>
            </Box>
          </ButtonBase>
        )}
      </Box>
