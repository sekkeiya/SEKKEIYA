  ...
  return (
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', bgcolor: 'background.default', overflowY: 'auto' }}>
      ...
      <Box sx={{ px: { xs: 3, md: 5 }, pt: 3 }}>
        <CardActionArea
          onClick={() => setCurrentMainView('team-site')}
          sx={{
            borderRadius: 2.5, border: '1px solid rgba(52,152,219,0.3)',
            bgcolor: 'rgba(52,152,219,0.07)', px: 2.5, py: 1.75,
            display: 'flex', alignItems: 'center', gap: 2,
            '&:hover': { bgcolor: 'rgba(52,152,219,0.12)', borderColor: 'rgba(52,152,219,0.5)' },
          }}
        >
          <Box sx={{ width: 38, height: 38, borderRadius: 2, bgcolor: 'rgba(52,152,219,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <PublicRoundedIcon sx={{ color: '#3498db', fontSize: 20 }} />
          </Box>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: BRAND.text }}>チームサイト</Typography>
            <Typography sx={{ fontSize: 12, color: BRAND.sub2 }}>
              チームの公開トップページ。実績（チームプロジェクト）をまとめて紹介します。
            </Typography>
          </Box>
          <Button
            variant="outlined" size="small" component="span"
            sx={{ borderColor: 'rgba(52,152,219,0.5)', color: '#3498db', fontSize: 12, fontWeight: 700, textTransform: 'none', borderRadius: 2, flexShrink: 0, '&:hover': { borderColor: '#3498db' } }}
          >
            編集する
          </Button>
        </CardActionArea>
