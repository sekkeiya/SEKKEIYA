// 索引商品の詳細モーダル右サイドバー。
// (A) この商品に視覚的に似た S.Models 3Dモデルを一覧（findSimilarModels=CLIP類似）。
...
export const SimilarModelsSidebar: React.FC<{ item: CatalogVisionItem }> = ({ item }) => {
  ...
  return (
    <Box sx={{ width: 300, flexShrink: 0, height: '100%', borderLeft: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ px: 2, pt: 2, pb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
        <ViewInArRoundedIcon sx={{ fontSize: 18, color: '#c4b5fd' }} />
        <Typography sx={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>似た3Dモデル</Typography>
      </Box>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 2, pb: 1 }}>
        {loading ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, py: 4 }}>
            ...
            <Typography sx={{ fontSize: 10.5, color: 'rgba(255,255,255,0.4)' }}>類似モデルを照合中…</Typography>
          </Box>
        ) : models.length > 0 ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
            {models.map((m) => (
              <Box key={m.id} onClick={openModels}
                sx={{ borderRadius: 2, overflow: 'hidden', bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', transition: 'border-color 0.15s', '&:hover': { borderColor: 'rgba(196,181,253,0.6)' } }}>
                <Box sx={{ position: 'relative', aspectRatio: '1/1', bgcolor: 'rgba(0,0,0,0.3)' }}>
                  ...
                  <Box sx={{ position: 'absolute', top: 4, right: 4, px: 0.75, py: 0.25, borderRadius: 1, bgcolor: 'rgba(124,58,237,0.9)', fontSize: 11, fontWeight: 700, color: '#fff' }}>
                    {Math.round(m.similarity * 100)}%
                  </Box>
                  {m.isLocal && (
                    <Box sx={{ position: 'absolute', top: 4, left: 4, px: 0.6, py: 0.2, borderRadius: 1, bgcolor: 'rgba(0,0,0,0.55)', fontSize: 9.5, fontWeight: 700, color: '#fcd34d' }}>
                      ローカル
                    </Box>
                  )}
                </Box>
                <Typography noWrap sx={{ fontSize: 11.5, px: 1, py: 0.75, color: 'rgba(255,255,255,0.88)' }}>{m.title}</Typography>
              </Box>
            ))}
          </Box>
        ) : (
          <Box sx={{ textAlign: 'center', py: 3, color: 'rgba(255,255,255,0.45)' }}>
      ...
      <Box sx={{ p: 2, pt: 1, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <Button
          fullWidth variant="contained" size="small" disabled={genBusy}
          startIcon={genBusy ? <CircularProgress size={14} sx={{ color: '#fff' }} /> : <AutoAwesomeRoundedIcon sx={{ fontSize: 16 }} />}
          onClick={generate}
          sx={{ bgcolor: '#7c3aed', '&:hover': { bgcolor: '#6d28d9' } }}
        >
          この商品から3Dモデルを生成
        </Button>
        <Typography sx={{ fontSize: 9.5, color: 'rgba(255,255,255,0.35)', mt: 0.75, textAlign: 'center' }}>
