          ...
          return (
            ...
            >
              <Box sx={{ position: 'relative', aspectRatio: '1/1', bgcolor: 'rgba(0,0,0,0.3)' }}>
                ...
                {m.similarity != null && (
                  <Box sx={{ position: 'absolute', top: 4, right: 4, px: 0.75, py: 0.25, borderRadius: 1, bgcolor: 'rgba(22,163,74,0.85)', fontSize: 10.5, fontWeight: 700, color: '#fff' }}>
                    {Math.round(m.similarity * 100)}%
                  </Box>
                )}
              </Box>
              <Box sx={{ p: 1 }}>
                <Typography noWrap sx={{ fontSize: 11.5, fontWeight: 600, color: '#fff' }}>
                  {isWeb ? (m.label || m.catalogTitle) : m.catalogTitle}
                </Typography>
                {isWeb ? (
                  <Typography noWrap sx={{ fontSize: 11, color: '#86efac', fontWeight: 700, mt: 0.25 }}>{m.price || ''}</Typography>
                ) : (
