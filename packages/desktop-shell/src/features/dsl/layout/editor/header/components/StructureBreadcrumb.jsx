...
// ヘッダーの Base / Plan パンくず。
//  - Base   … ドロップダウンではなく「躯体編集モードのトグル」。
//             通常時にクリック → 躯体のみ表示（Plan を解除）。
//             躯体モード中にクリック → 直前に開いていた Plan へ戻る。
//             別 Base への切替はここではできない（Exit → Layout Dashboard から選ぶ）。
//  - Plan   … 選択中 Base 配下の Plan を選択／新規作成（家具配置を複数作れる）
// 提案（旧 Option）はこのパンくずの範囲外。パンくずと同じ Row に独立した
// ProposalSelector（TopBar.jsx 参照）として表示する — Base/Plan の階層軸とは
// 別の軸（見た目のバリエーション）なので、パンくずのセグメントにはしない。
// Plan 未選択（=Base のみ開いている）ときは躯体編集バッジを表示。
const menuPaperSx = {
...
export default function StructureBreadcrumb() {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 0.25, minWidth: 0, overflow: "hidden" }}>
      ...
      <Segment label={planName ? planName : "Plan: 未選択"} strong={!!planName} onClick={openMenu("plan")} />
