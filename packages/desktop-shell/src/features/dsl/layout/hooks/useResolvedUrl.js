...
// localStorage 永続キャッシュ（アプリ再起動後も有効）
const LS_PREFIX = 'skky_uc_';
const LS_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7日
...
try {
    for (let i = localStorage.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            if (!raw) { localStorage.removeItem(lsKey); continue; }
            const entry = JSON.parse(raw);
            if (!entry?.ok || isExpired(entry, LS_TTL_MS)) {
                localStorage.removeItem(lsKey);
            } else {
                URL_CACHE.set(lsKey.slice(LS_PREFIX.length), entry);
            }
...
export async function resolveUrlAsync(raw, version = 0) {
    ...
    try {
        ...
        if (isHttp) {
            if (isFsHttp && !hasToken) {
                ...
                setCacheEntry(key, { value: out, ts: now(), ok: true });
                return out;
            }
