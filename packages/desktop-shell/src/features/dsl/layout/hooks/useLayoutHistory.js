...
function areLayoutsCloseEnough(layout1, layout2) {
    ...
    for (let i = 0; i < items1.length; i++) {
        ...
        if (item1 === item2) continue; // Optimization: Same reference
...
export function useLayoutHistory({ optionDoc, optionDocLoading, setDirty, historyLimit = 100 }) {
    ...
    const prevOptionIdRef = useRef(optionId);

    useEffect(() => {
        if (optionDocLoading) return;
        
        // Only reset if the option ID changes (e.g. switching options)
        if (optionId !== prevOptionIdRef.current) {
            setLayoutDraft(initial);
            undoStackRef.current = [];
            redoStackRef.current = [];
            batchingRef.current = false;
            batchBeforeRef.current = null;
            prevOptionIdRef.current = optionId;
            if (setDirty) setDirty(false);
        }
    }, [optionId, initial, optionDocLoading, setDirty]);
    ...
    const beginBatch = useCallback(() => {
        ...
        setLayoutDraft((currentLayout) => {
            batchBeforeRef.current = currentLayout;
    ...
    const endBatch = useCallback(() => {
        ...
        setLayoutDraft((currentLayout) => {
            ...
            if (before) {
                pushHistory(before, currentLayout);
            }
            return currentLayout;
        });
    }, [pushHistory]);
