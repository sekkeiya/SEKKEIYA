...
export default function TransformGizmo({
  ...
  // 選択オブジェクトから pivot 行列・オフセットを初期化する純粋関数。
  // ⚠️ useEffect は描画後に走るため、PivotControls が新オブジェクトで
  //    マウントされる前に pivotMatrixRef を更新できず、前オブジェクトの
  //    行列を掴んだまま固定されてしまう。これを防ぐため「描画フェーズ」で
  //    同期する（下の useMemo 相当のガード参照）。
  const initPivotFromObject = useCallback((obj) => {
    if (!obj) return;
    ...
    // ⚠️ キャッシュを新オブジェクト基準で即座にプライムする。
    //   useFrame は matrixWorld が「前回値と等しい」と再計算をスキップし
    //   cachedPivotPos（＝前アイテムの pivot）を使い続けてしまう。新アイテムが
    //   原点付近に置かれた場合に「前アイテムの基点にギズモが残る」原因。
    cachedPivotPosRef.current.copy(worldPivotPos);
    lastMatrixWorldRef.current.copy(obj.matrixWorld);

    // 初期表示用に pivot 行列も worldPivotPos を起点に組んでおく（次フレームで useFrame が上書き）
    pivotMatrixRef.current.identity();
    pivotMatrixRef.current.setPosition(worldPivotPos);
  }, []);

  // ── 描画フェーズ同期 ──
  // selectedObject が変わった瞬間に（子の PivotControls がマウントされる前に）
  // pivot 行列を新オブジェクト基準へ更新しておく。これにより、アイテム追加直後の
  // 「前アイテムの基点にギズモが残る」不具合を防ぐ。
  const prevSelForInitRef = useRef(null);
  if (selectedObject !== prevSelForInitRef.current) {
    prevSelForInitRef.current = selectedObject;
    if (selectedObject) initPivotFromObject(selectedObject);
  }
  ...
  useEffect(() => {
    ...
    // 念のため再初期化（GLB ロード遅延などで box が変化した場合に追従）
    if (selectedObject) initPivotFromObject(selectedObject);
  }, [selectedObject, initPivotFromObject]);
  ...
  return isVisible ? (
    <group
      // 選択オブジェクトが変わったら PivotControls をクリーン再マウントし、
      // 確実に新オブジェクトの pivot 行列で初期化させる（前アイテムの基点残り対策）。
      key={selectedObject?.uuid || 'gizmo'}
      ref={gizmoRootRef}
      onClick={handleClick}
