...
import { useFurnitureDimensionPrefsStore } from "../../store/useFurnitureDimensionPrefsStore";
...
function defaultAdoptedKeys(segs) {
  ...
  let v = null;
  for (const s of segs) {
    if (s.axis === "h") { if (!h || s.mm < h.mm) h = s; }
...
export default function FurnitureDimensionOverlay() {
  ...
  useEffect(() => {
    ...
    recompute();
    // ドラッグ中は下の useFrame が 15fps で実時間更新するため、ここはドラッグ外の
    // 変化(undo/数値入力/オートレイアウト)を拾うアイドルポーリング。150ms は過剰
    // (毎秒6.7回の全オブジェクト計測)だったので 500ms に緩めて CPU を削減。
