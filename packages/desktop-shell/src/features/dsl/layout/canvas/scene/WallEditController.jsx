...
export default function WallEditController({ enabled = true, orbitRef = null, sideAxis = null }) {
  ...
  // 断面(gizmoOnly)の壁リサイズ: "top"=高さ(heightMm) / "bottom"=足元(offsetYMm・上端保持) /
  //   "left"/"right"=壁厚(thicknessMm・壁芯対称)。スラブの断面ハンドルと同じ操作感。
  const [secResize, setSecResize] = useState(null);
  const secResizeOrigRef = useRef(null);
  ...
  // 断面(gizmoOnly)での壁リサイズ。上端=高さ / 下端=足元(上端保持) / 左右=壁厚(壁芯対称)。
  //   スラブの断面ハンドルと同じく、視線に直交する鉛直面へレイキャストして world Y / 横位置(mm)を取る。
  useEffect(() => {
    if (!secResize || !gizmoOnly) return;
    const el = gl.domElement;
    ...
    const hit = new THREE.Vector3();
    const onMove = (ev) => {
      const o = secResizeOrigRef.current; if (!o) return;
      ...
      if (secResize === "top") {
        let yMm = hit.y / k;
        yMm = ev.shiftKey ? Math.round(yMm / 50) * 50 : Math.round(yMm);
        const height = Math.max(50, Math.round(yMm - o.baseY)); // 足元固定で高さを変える
        st.updateWallLocal(o.wallId, { heightMm: height });
      } else if (secResize === "bottom") {
        let yMm = hit.y / k;
        yMm = ev.shiftKey ? Math.round(yMm / 50) * 50 : Math.round(yMm);
        const newBase = Math.min(yMm, o.topY - 50); // 上端は保持（最低高さ50mm）
        const height = Math.max(50, Math.round(o.topY - newBase));
        const offsetY = Math.round(newBase - o.floorBaseYMm);
        st.updateWallLocal(o.wallId, { offsetYMm: offsetY, heightMm: height });
      } else {
        // left/right = 壁厚（壁芯を中心に対称で伸縮）
        const hMm = (sideAxis === "x" ? hit.x : hit.z) / k;
        let t = Math.max(50, Math.round(2 * Math.abs(hMm - o.centerH)));
        if (ev.shiftKey) t = Math.max(50, Math.round(t / 50) * 50);
        st.updateWallLocal(o.wallId, { thicknessMm: t });
      }
    };
    const onUp = () => {
      useWallStore.getState().persistWalls();
  ...
  }, [secResize, gizmoOnly, sideAxis, camera, gl, k]);
  ...
  return (
    ...
    <group userData={{ ignoreClipping: true, isDrawnStructure: true }}>
      ...
      {/* 断面(gizmoOnly)の壁リサイズハンドル: 上端=高さ / 下端=足元 / 左右=壁厚。単体選択時のみ。
          左右(厚み)は「断面に垂直に切られた壁」＝黒バーの横幅が壁厚のときだけ出す
          （画面横に走る壁は横=長さになり厚み操作にならないため）。 */}
      {gizmoOnly && wall && selectedWallIds.length === 1 && (() => {
        const fi = Math.max(0, Math.min(wall.floorIndex || 0, (floors?.length || 1) - 1));
        ...
        const uSide = Math.abs((sideAxis === "x" ? dxu : dzu) / ulen);
        const showThickness = uSide < 0.5; // 視線軸沿い＝断面に垂直な壁のみ厚みハンドル
        const t = wall.thicknessMm || 100;
        const leftH = centerH - t / 2, rightH = centerH + t / 2;
        const begin = (mode) => (e) => {
          if (e.button !== 0) return;
          e.stopPropagation();
          useViewportUiStore.getState().setGizmoDragging?.(true);
          secResizeOrigRef.current = {
            wallId: wall.id,
            floorBaseYMm, baseY, topY, centerH,
            planeAt: (sideAxis === "x" ? cz : cx) * k,
          };
          setSecResize(mode);
        };
