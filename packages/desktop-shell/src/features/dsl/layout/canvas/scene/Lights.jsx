...
function useGizmoHandlers(lightId) {
  ...
  const onClick = useCallback(
    (e) => {
      e.stopPropagation();
      if (!canSelectLight(useSelectionScopeStore.getState().scope)) return;
      // ゾーン選択中は Light を選択しない（1クリック目でゾーン選択解除）
      const taskStore = useLayoutTaskStore.getState();
      if (taskStore.activeZoneId) {
        taskStore.setActiveZoneId(null);
        return;
      }
      selectLight(lightId);
...
function SpotLightRenderer({ light }) {
  ...
  // 重要: スポットのシャドウカメラは透視投影。near が極端に小さい(=1)と
  // near/far 比が数千倍になり深度バッファの精度が壊れ、影が描画されない
  // （directional は平行投影なので精度が線形で影が出る）。
  // ライト高さに比例した near を取り、近接面より手前にだけ置く。
  const mmScale = pos[1] > 100; // mm シーン判定（位置=シーン単位）
  const shadowNear = Math.max(pos[1] * 0.08, mmScale ? 50 : 0.05);
  // normalBias もスケール依存。mm シーンでは数 mm 必要（1.5 では小さすぎ acne/欠落）。
  const shadowNormalBias = mmScale ? 8 : 0.04;
...
function SpotLightFootprint({ light }) {
  ...
  if (shadowsEnabled) {
    return (
      <group position={[cx, 5, cz]} userData={{ isLightFootprint: true }}>
...
function NeonLightFootprint({ light }) {
  ...
  return (
    <group position={[px, 5, pz]} rotation={[0, yRotRad, 0]} userData={{ isLightFootprint: true }}>
      {/* 外周ソフトハロー */}
      <mesh rotation={[-Math.PI / 2, 0, 0]} renderOrder={95} userData={{ ignoreClipping: true }}>
...
export default function Lights({ hasBase = true, hideGizmo = false }) {
  ...
  const showFootprints = !isWalkthrough && scope !== "map";
        ...
        if (light.type === "spot") {
          ...
          return (
            <React.Fragment key={light.id}>
              ...
              {/* 床への照射エリア: ウォークスルー以外で表示（作図ヘルパ） */}
              {showFootprints && <SpotLightFootprint light={light} />}
        ...
        if (light.type === "rect") {
          ...
          return (
            <React.Fragment key={light.id}>
              ...
              {/* 床への照射エリア: ウォークスルー以外で表示（作図ヘルパ） */}
              {showFootprints && <RectAreaFootprint light={light} />}
              {/* ギズモ: ALL / Lighting のみ */}
              {showGizmos && (
                <RectAreaGizmo
                  lightId={light.id}
        ...
        if (light.type === "neon") {
          ...
          return (
            <React.Fragment key={light.id}>
              ...
              {/* 床への照射エリア: ウォークスルー以外で表示（作図ヘルパ） */}
              {showFootprints && <NeonLightFootprint light={light} />}
              {/* ギズモ: ALL / Lighting のみ */}
              {showGizmos && (
                <NeonLightGizmo
                  lightId={light.id}
