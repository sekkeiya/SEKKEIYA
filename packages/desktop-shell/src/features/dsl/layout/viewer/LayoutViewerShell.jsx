...
import WalkthroughViewerCanvas from "./viewport/WalkthroughViewerCanvas.jsx";
...
export default function LayoutViewerShell({
  ...
  // ✅ ウォークスルー（操作可能）モード。?walkthrough=1 か viewerConfig.walkthrough.enabled で起動。
  const walkthroughCfg = viewerConfig?.walkthrough || null;
  const walkthroughEnabled = (searchParams.get("walkthrough") === "1" || !!walkthroughCfg?.enabled) && !!walkthroughCfg;

  // ✅ 本番プレビュー（鑑賞ビューワ）モード。?presentation=1 か viewerConfig.presentation.enabled で起動。
  const presentationCfg = viewerConfig?.presentation || null;
  const presentationEnabled =
    searchParams.get("presentation") === "1" || !!presentationCfg?.enabled;
  ...
  // ✅ 本番プレビュー（鑑賞ビューワ）: フルスクリーンで PresentationViewer を表示
  if (presentationEnabled) {
    return (
      <PresentationViewer
        open
        onClose={handleHome}
        baseGlbUrl={presentationCfg?.baseGlbUrl || baseGlbUrlResolved || ""}
        roomSpec={presentationCfg?.roomSpec || null}
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minHeight: 0, display: "grid", gridTemplateColumns: "320px 1fr 360px" }}>
        ...
        <Box sx={{ minHeight: 0 }}>
          {walkthroughEnabled ? (
            <WalkthroughViewerCanvas
              walkthrough={{
                ...walkthroughCfg,
                baseGlbUrl: baseGlbUrlResolved || walkthroughCfg.baseGlbUrl || "",
              }}
              layout={optionDoc?.layout || null}
            />
          ) : (
