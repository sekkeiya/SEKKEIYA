...
export const WorkFilesList: React.FC<WorkFilesListProps> = ({ project, filterMode = 'all', externalSelectedFileId, onSelectedFileChange }) => {
  ...
  const getGroupNote = (key: string): string | null => {
    if (key === '3dsp') return 'S.Slideで作業したファイルが自動的に登録されます';
  ...
  const getStatusInfo = (fileId: string) => {
    ...
    if (!st) return { label: '確認中', color: 'rgb(var(--brand-fg-rgb) / 0.3)', bg: 'rgb(var(--brand-fg-rgb) / 0.04)', badge: null };
    ...
    return { label: 'クラウドのみ', color: 'rgb(var(--brand-fg-rgb) / 0.45)', bg: 'rgb(var(--brand-fg-rgb) / 0.04)', badge: 'cloud_only' };
  };
  ...
  return (
    <Box sx={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden', position: 'relative' }}>
      ...
      {filterMode === 'cad' ? (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, mb: 1.5, pb: 1.5, borderBottom: '1px solid rgb(var(--brand-fg-rgb) / 0.06)', flexShrink: 0 }}>
          {/* Row 1: title + file count + action buttons */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', rowGap: 0.75 }}>
            <FolderOpenRoundedIcon sx={{ fontSize: 18, color: 'light-dark(#a80637, #fa709a)' }} />
            <Typography sx={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--brand-fg)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 320 }}>
              {project.name}
            </Typography>
            <Typography sx={{ fontSize: '0.65rem', color: 'rgb(var(--brand-fg-rgb) / 0.3)', flexShrink: 0 }}>
            ...
            <Box sx={{ display: 'flex', gap: 0.75 }}>
              <Button size="small" variant="contained" onClick={() => setIsTemplateDialogOpen(true)} disabled={uploading}
                ...
                sx={{ bgcolor: '#fa709a', color: 'var(--brand-fg)', fontWeight: 700, textTransform: 'none', fontSize: '0.72rem', px: 1.5, py: 0.5, whiteSpace: 'nowrap', '&:hover': { bgcolor: '#ff90b2' } }}>
                テンプレート
              </Button>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', rowGap: 0.75 }}>
              ...
              {(['all', 'rhino', 'blender'] as const).map(t => (
                <Button key={t} size="small" onClick={() => setToolTypeFilter(t)}
                  sx={{ textTransform: 'none', fontWeight: toolTypeFilter === t ? 700 : 500, fontSize: '0.72rem', px: 1.25, py: 0.375, minWidth: 0, borderRadius: 1.5,
                    color: toolTypeFilter === t ? 'var(--brand-fg)' : 'rgb(var(--brand-fg-rgb) / 0.45)',
            ...
            <Box sx={{ display: 'flex', gap: 0.25, p: 0.375, bgcolor: 'light-dark(rgba(15,23,42,0.12), rgba(0,0,0,0.35))', borderRadius: 2, border: '1px solid rgb(var(--brand-fg-rgb) / 0.09)' }}>
              <Tooltip title="グリッド表示">
                <IconButton size="small" onClick={() => setViewMode('grid')}
                  sx={{ p: 0.5, borderRadius: 1.5, color: viewMode === 'grid' ? 'var(--brand-fg)' : 'rgb(var(--brand-fg-rgb) / 0.4)',
                    bgcolor: viewMode === 'grid' ? 'rgb(var(--brand-fg-rgb) / 0.1)' : 'transparent',
                    '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.07)', color: 'var(--brand-fg)' } }}>
              ...
              <Tooltip title="リスト表示">
                <IconButton size="small" onClick={() => setViewMode('list')}
                  sx={{ p: 0.5, borderRadius: 1.5, color: viewMode === 'list' ? 'var(--brand-fg)' : 'rgb(var(--brand-fg-rgb) / 0.4)',
                    bgcolor: viewMode === 'list' ? 'rgb(var(--brand-fg-rgb) / 0.1)' : 'transparent',
                    '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.07)', color: 'var(--brand-fg)' } }}>
              ...
              <Tooltip title="ローカル保存先を確認">
                <IconButton onClick={() => setFolderSettingsDialogOpen(true)} size="small"
                  sx={{ p: 0.5, borderRadius: 1.5, color: 'rgb(var(--brand-fg-rgb) / 0.4)',
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgb(var(--brand-fg-rgb) / 0.08)', pb: 1.5, mb: 2, flexShrink: 0 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <Tooltip title="ローカル保存先を確認">
              <IconButton onClick={() => setFolderSettingsDialogOpen(true)} size="small"
                sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5,
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', pr: 0.5 }}>
          {loading ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pt: 12, gap: 2 }}>
              <AutoAwesomeRoundedIcon sx={{ fontSize: 56, color: 'rgb(var(--brand-fg-rgb) / 0.12)' }} />
              <Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.35)', textAlign: 'center', lineHeight: 2 }}>
                ファイルがありません<br />
                <Box component="span" sx={{ fontSize: '0.85rem', color: 'rgb(var(--brand-fg-rgb) / 0.25)' }}>
                  {filterMode === 'other'
                    ? 'S.Layout や S.Slide で作業すると、ここに自動登録されます'
            ...
            Object.entries(groupedFiles).map(([groupKey, groupFileList]) => (
              <Box key={groupKey} sx={{ mb: 4 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  <Typography sx={{ fontSize: '0.68rem', fontWeight: 800, color: 'rgb(var(--brand-fg-rgb) / 0.4)', letterSpacing: 1.5, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    {getGroupDisplayLabel(groupKey)}
                  </Typography>
                  {getGroupNote(groupKey) && (
                    <Typography sx={{ fontSize: '0.63rem', color: 'rgb(var(--brand-fg-rgb) / 0.22)', whiteSpace: 'nowrap' }}>
                      — {getGroupNote(groupKey)}
                    </Typography>
                  )}
                  <Divider sx={{ flex: 1, borderColor: 'rgb(var(--brand-fg-rgb) / 0.07)' }} />
                  <Typography sx={{ fontSize: '0.63rem', color: 'rgb(var(--brand-fg-rgb) / 0.22)', whiteSpace: 'nowrap' }}>{groupFileList.length}件</Typography>
                </Box>
                      ...
                      return (
                        <Box key={file.id} onClick={() => setSelectedFileId(file.id)}
                          sx={{ width: 192, borderRadius: 2.5, overflow: 'hidden', cursor: 'pointer', position: 'relative',
                            border: isSelected ? '1.5px solid rgba(0,191,255,0.7)' : '1px solid rgb(var(--brand-fg-rgb) / 0.08)',
                            bgcolor: isSelected ? 'rgba(0,191,255,0.06)' : 'rgb(var(--brand-fg-rgb) / 0.02)',
                          ...
                          {/* Thumbnail */}
                          <Box sx={{ aspectRatio: '16/9', bgcolor: 'var(--brand-bg)', position: 'relative', overflow: 'hidden' }}>
                            {file.thumbnailUrl
                            ...
                            <Box sx={{ position: 'absolute', top: 6, right: 6, px: 0.75, py: 0.2, borderRadius: 1, bgcolor: st.bg, border: `1px solid color-mix(in srgb, ${st.color} 33%, transparent)` }}>
                          ...
                          <Box sx={{ px: 1.5, pt: 1.25, pb: 1.5 }}>
                            <Tooltip title={file.name} placement="top" enterDelay={600}>
                              <Typography noWrap sx={{ fontWeight: 700, fontSize: '0.82rem', color: isSelected ? 'var(--brand-fg)' : 'rgb(var(--brand-fg-rgb) / 0.85)', lineHeight: 1.3, mb: 0.75 }}>
                                {file.name}
                              </Typography>
                            </Tooltip>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <Chip size="small" label={`v${file.latestVersionNumber || 1}`} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.07)', color: 'rgb(var(--brand-fg-rgb) / 0.5)' }} />
                      ...
                      return (
                        <Box key={file.id} onClick={() => setSelectedFileId(file.id)}
                          sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 1.5, py: 1, borderRadius: 2, cursor: 'pointer',
                            border: `1px solid ${isSelected ? 'rgba(0,191,255,0.4)' : 'rgb(var(--brand-fg-rgb) / 0.06)'}`,
                            bgcolor: isSelected ? 'rgba(0,191,255,0.05)' : 'transparent',
                            transition: 'all 0.15s',
                            '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.03)', borderColor: 'rgb(var(--brand-fg-rgb) / 0.12)', '& .list-actions': { opacity: 1 } } }}>
                          {/* Thumbnail */}
                          <Box sx={{ width: 72, height: 40, bgcolor: 'var(--brand-bg)', borderRadius: 1.5, flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                            {file.thumbnailUrl
                              ? <Box component="img" src={file.thumbnailUrl} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Typography noWrap sx={{ fontWeight: 700, fontSize: '0.85rem', color: isSelected ? 'var(--brand-fg)' : 'rgb(var(--brand-fg-rgb) / 0.88)' }}>
                              {file.name}
                            </Typography>
                            <Typography sx={{ fontSize: '0.62rem', color: 'rgb(var(--brand-fg-rgb) / 0.38)' }}>
                              {getFileTypeLabel(file)}
                            </Typography>
                          </Box>
                          {/* Version */}
                          <Chip size="small" label={`v${file.latestVersionNumber || 1}`}
                            sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.07)', color: 'rgb(var(--brand-fg-rgb) / 0.5)', flexShrink: 0 }} />
                          {/* Status */}
                          <Box sx={{ px: 0.75, py: 0.2, borderRadius: 1, bgcolor: st.bg, border: `1px solid color-mix(in srgb, ${st.color} 33%, transparent)`, flexShrink: 0 }}>
                            <Typography sx={{ fontSize: '0.6rem', fontWeight: 800, color: st.color, lineHeight: 1.4 }}>{st.label}</Typography>
                          </Box>
                          {/* Date */}
                          <Typography sx={{ fontSize: '0.65rem', color: 'rgb(var(--brand-fg-rgb) / 0.3)', flexShrink: 0, minWidth: 48, textAlign: 'right' }}>
                            {new Date(file.updatedAt).toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' })}
                          </Typography>
                          {/* Actions */}
      ...
      {selectedFile && (
        <Box sx={{ width: 320, flexShrink: 0, borderLeft: '1px solid rgb(var(--brand-fg-rgb) / 0.08)', bgcolor: 'var(--brand-surface)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid rgb(var(--brand-fg-rgb) / 0.07)', display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
              ...
              <Typography sx={{ fontWeight: 800, fontSize: '0.85rem', color: 'var(--brand-fg)', flex: 1 }}>ファイル詳細</Typography>
              <Tooltip title="名前を変更">
                <IconButton onClick={handleEditWorkFileOpen} size="small" sx={{ p: '4px', color: 'rgb(var(--brand-fg-rgb) / 0.3)', '&:hover': { color: 'var(--brand-fg)' } }}>
                  <EditRoundedIcon sx={{ fontSize: 14 }} />
                </IconButton>
              </Tooltip>
              <Tooltip title="このファイルを削除">
                <IconButton onClick={handleDeleteWorkFileClick} size="small" sx={{ p: '4px', color: 'rgb(var(--brand-fg-rgb) / 0.3)', '&:hover': { color: 'light-dark(#a80637, #fa709a)' } }}>
                  <DeleteOutlineRoundedIcon sx={{ fontSize: 14 }} />
                </IconButton>
              </Tooltip>
              <IconButton onClick={() => setSelectedFileId(null)} size="small" sx={{ p: '4px', color: 'rgb(var(--brand-fg-rgb) / 0.4)', '&:hover': { color: 'var(--brand-fg)' } }}>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', px: 2.5, py: 1.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              {/* File name field */}
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>ファイル名</Typography>
                <Box sx={{ px: 1.25, py: 0.875, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.03)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5 }}>
                  <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--brand-fg)', wordBreak: 'break-all', lineHeight: 1.4 }}>{selectedFile.name}</Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 1.5 }}>
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>種別</Typography>
                  <Box sx={{ px: 1.25, py: 0.75, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.03)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5 }}>
                    <Typography sx={{ fontSize: '0.75rem', color: 'rgb(var(--brand-fg-rgb) / 0.8)' }}>{getFileTypeLabel(selectedFile)}</Typography>
                  </Box>
                </Box>
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>バージョン</Typography>
                  <Box sx={{ px: 1.25, py: 0.75, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.03)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5 }}>
                    <Typography sx={{ fontSize: '0.75rem', color: 'rgb(var(--brand-fg-rgb) / 0.8)' }}>v{selectedFile.latestVersionNumber || 1}</Typography>
                  </Box>
                </Box>
              </Box>
              ...
              {/* Status */}
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>ステータス</Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1.5 }}>
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 0.4 }}>
                  <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>最終更新</Typography>
                  <Typography sx={{ fontSize: '0.75rem', color: 'rgb(var(--brand-fg-rgb) / 0.75)', fontWeight: 500 }}>
                ...
                <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 0.4 }}>
                  <Typography sx={{ fontSize: '0.72rem', color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, letterSpacing: 0.3 }}>更新者</Typography>
                  <Typography noWrap sx={{ fontSize: '0.75rem', color: 'rgb(var(--brand-fg-rgb) / 0.75)', fontWeight: 500 }}>{updatedByName || selectedFile.updatedBy}</Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 0.75 }}>
                <Button fullWidth variant="outlined" startIcon={<FileCopyRoundedIcon />} onClick={handleDuplicateOpenDialog}
                  sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', borderColor: 'rgb(var(--brand-fg-rgb) / 0.15)', fontSize: '0.75rem', textTransform: 'none', borderRadius: 1.5,
                    '&:hover': { borderColor: '#00BFFF', bgcolor: 'rgba(0,191,255,0.08)', color: 'var(--brand-fg)' } }}>
                  別案として複製
                </Button>
                <Tooltip title="複製分岐について">
                  <IconButton onClick={() => setInfoDialogType('fork')}
                    sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.4)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5, p: 0.625,
              ...
              <Button variant="text" size="small" onClick={() => toggleExpand(selectedFile.id)} startIcon={<ViewListRoundedIcon />}
                sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.4)', fontWeight: 600, textTransform: 'none', justifyContent: 'flex-start', fontSize: '0.75rem',
                  '&:hover': { color: 'var(--brand-fg)', bgcolor: 'transparent' } }}>
              ...
              {expandedFiles[selectedFile.id] && (
                <Box>
                  <Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.3)', mb: 1, fontWeight: 800, fontSize: '0.63rem', letterSpacing: 1.5, textTransform: 'uppercase' }}>ローカル履歴</Typography>
                  {(versionsMap[selectedFile.id] || []).length === 0
                    ? <Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.3)', fontSize: '0.75rem', py: 1 }}>ローカル履歴がありません</Typography>
                    : (
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        {(versionsMap[selectedFile.id] || []).map((v, idx) => (
                          <Box key={v.name} sx={{ display: 'flex', alignItems: 'center', p: 1, borderRadius: 1.5, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.02)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.05)', '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.05)', '& .ver-actions': { opacity: 1 } } }}>
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              <Typography noWrap sx={{ fontSize: '0.73rem', color: 'rgb(var(--brand-fg-rgb) / 0.75)', fontWeight: idx === 0 ? 700 : 400 }}>{v.name}</Typography>
                              <Typography sx={{ fontSize: '0.63rem', color: 'rgb(var(--brand-fg-rgb) / 0.35)' }}>{idx === 0 ? '最新' : '過去のコピー'} • {formatDateTimeSec(v.createdAt)}</Typography>
                            </Box>
                            <Box className="ver-actions" sx={{ display: 'flex', opacity: 0, transition: 'opacity 0.15s', gap: 0.25, flexShrink: 0, ml: 0.5 }}>
                              <Tooltip title="プレビュー"><IconButton size="small" onClick={(e) => openVersionPreview(e, selectedFile, v.path)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', p: 0.4, '&:hover': { color: '#00BFFF' } }}><VisibilityRoundedIcon sx={{ fontSize: '0.95rem' }} /></IconButton></Tooltip>
                              <Tooltip title="この版を開く"><IconButton size="small" onClick={(e) => { e.stopPropagation(); handleOpenLocal(selectedFile, v.path); }} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', p: 0.4, '&:hover': { color: '#00BFFF' } }}><PlayArrowRoundedIcon sx={{ fontSize: '0.95rem' }} /></IconButton></Tooltip>
                              <Tooltip title="名前を変更"><IconButton size="small" onClick={(e) => handleEditVersionClick(e, selectedFile.id, v.path, v.name)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', p: 0.4, '&:hover': { color: '#00BFFF' } }}><EditRoundedIcon sx={{ fontSize: '0.95rem' }} /></IconButton></Tooltip>
                              <Tooltip title="削除"><IconButton size="small" onClick={(e) => handleDeleteVersionClick(e, selectedFile.id, v.path)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', p: 0.4, '&:hover': { color: 'light-dark(#a80637, #fa709a)' } }}><DeleteOutlineRoundedIcon sx={{ fontSize: '0.95rem' }} /></IconButton></Tooltip>
                            </Box>
                          </Box>
                        ))}
                      </Box>
            ...
            {/* Footer */}
            <Box sx={{ px: 2.5, py: 1.75, borderTop: '1px solid rgb(var(--brand-fg-rgb) / 0.07)', display: 'flex', gap: 1, justifyContent: 'flex-end', alignItems: 'center', flexShrink: 0 }}>
              <Button onClick={() => setSelectedFileId(null)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', textTransform: 'none', fontWeight: 600, fontSize: '0.78rem' }}>
                閉じる
              </Button>
              <Tooltip title="クラウド連携について">
                <IconButton onClick={() => setInfoDialogType('upload')}
                  sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.4)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.12)', borderRadius: 1.5, p: 0.5,
                    '&:hover': { borderColor: '#fa709a', color: 'light-dark(#a80637, #fa709a)' } }}>
                  <InfoOutlinedIcon sx={{ fontSize: 14 }} />
                </IconButton>
              </Tooltip>
              <Button onClick={() => handleUploadVersion(selectedFile)} variant="contained"
                sx={{ bgcolor: '#fa709a', color: 'var(--brand-fg)', fontWeight: 700, textTransform: 'none', borderRadius: 2, fontSize: '0.78rem',
      ...
      <Dialog open={cloudOverwriteDialogOpen} onClose={() => setCloudOverwriteDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface)', backgroundImage: 'none', border: '1px solid rgb(var(--brand-fg-rgb) / 0.1)', borderRadius: 3, maxWidth: 600 } }}>
        <DialogTitle sx={{ color: 'light-dark(#a80637, #fa709a)', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 1.5, pb: 2 }}><WarningAmberRoundedIcon /> ローカルの未保存な変更があります</DialogTitle>
        <DialogContent>
          <Typography sx={{ mb: 2, color: 'rgb(var(--brand-fg-rgb) / 0.8)' }}>手元のローカルファイルは、最後にクラウドへ保存した「版」よりも新しく編集されています。<br />クラウドから版をダウンロードすると、<b>現在のローカルの変更内容は上書きされて失われます</b>。</Typography>
          <Box sx={{ p: 2, bgcolor: 'rgba(0,191,255,0.05)', borderLeft: '4px solid #00BFFF', borderRadius: '0 8px 8px 0', mb: 2 }}><Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', fontSize: '0.9rem' }}>安全のため、先に現在の状態を「新しい版として保存」することをお勧めします。</Typography></Box>
          <Typography variant="body2" sx={{ color: 'light-dark(#a80637, #fa709a)', fontWeight: 'bold' }}>本当にクラウドの版をダウンロードして上書きしてもよろしいですか？</Typography>
        </DialogContent>
        <DialogActions sx={{ p: 3, pt: 0, display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
          <Button onClick={() => setCloudOverwriteDialogOpen(false)} variant="outlined" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)', borderRadius: 2 }}>キャンセル</Button>
          ...
          <Button onClick={handleCloudOverwriteConfirm} variant="contained" sx={{ bgcolor: 'rgba(250,112,154,0.1)', color: 'light-dark(#a80637, #fa709a)', fontWeight: 'bold', borderRadius: 2, px: 3, boxShadow: 'none', '&:hover': { bgcolor: 'rgba(250,112,154,0.2)', boxShadow: 'none' } }}>放棄して上書きダウンロード</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={duplicateDialogOpen} onClose={() => setDuplicateDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', borderRadius: 3, minWidth: 400 } }}>
        <DialogTitle sx={{ fontWeight: 800 }}>別案として複製</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
          <Box>
            <Typography variant="body2" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.6)', mb: 1 }}>複製元: {selectedFile?.name}</Typography>
            <Chip size="small" label={hasLocal ? 'ローカル元 (リンク済み)' : 'クラウド元 (未実装)'} sx={{ bgcolor: hasLocal ? 'rgba(67,233,123,0.1)' : 'rgb(var(--brand-fg-rgb) / 0.1)', color: hasLocal ? '#43e97b' : 'var(--brand-fg)' }} />
          </Box>
          <TextField fullWidth label="新しいファイル名" value={duplicateName} onChange={(e) => setDuplicateName(e.target.value)} InputLabelProps={{ sx: { color: 'rgb(var(--brand-fg-rgb) / 0.5)' } }} InputProps={{ sx: { color: 'var(--brand-fg)', '& fieldset': { borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)' } } }} />
          <TextField fullWidth label="初回コメント" value={duplicateComment} onChange={(e) => setDuplicateComment(e.target.value)} InputLabelProps={{ sx: { color: 'rgb(var(--brand-fg-rgb) / 0.5)' } }} InputProps={{ sx: { color: 'var(--brand-fg)', '& fieldset': { borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)' } } }} />
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setDuplicateDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', textTransform: 'none' }}>キャンセル</Button>
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', borderRadius: 3, minWidth: 400 } }}>
        <DialogTitle sx={{ fontWeight: 800 }}>ファイル名を変更</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
          <TextField fullWidth label="ファイル名" autoFocus value={editName} onChange={(e) => setEditName(e.target.value)} InputLabelProps={{ sx: { color: 'rgb(var(--brand-fg-rgb) / 0.5)' } }} InputProps={{ sx: { color: 'var(--brand-fg)', '& fieldset': { borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)' } } }} onKeyDown={(e) => e.key === 'Enter' && handleEditWorkFileSubmit()} />
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setEditDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', textTransform: 'none' }}>キャンセル</Button>
      ...
      <Dialog open={pastVersionSaveDialogOpen} onClose={() => setPastVersionSaveDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', borderRadius: 3, minWidth: 400 } }}>
        <DialogTitle sx={{ fontWeight: 800 }}>過去版からの保存</DialogTitle>
        <DialogContent sx={{ pt: 1 }}><Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.8)', mb: 2 }}>現在、過去の版を開いて作業しています。どのように保存しますか？</Typography></DialogContent>
        <DialogActions sx={{ p: 2, pt: 0, flexDirection: 'column', gap: 1 }}>
          ...
          <Button fullWidth variant="outlined" onClick={() => { setPastVersionSaveDialogOpen(false); handleDuplicateOpenDialog(); }} sx={{ borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)', color: 'var(--brand-fg)', fontWeight: 700 }}>別案として新しいファイルを作成</Button>
          <Button fullWidth onClick={() => setPastVersionSaveDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', mt: 1 }}>キャンセル</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', borderRadius: 3, minWidth: 400 } }}>
        <DialogTitle sx={{ fontWeight: 800, color: 'light-dark(#a80637, #fa709a)', display: 'flex', alignItems: 'center', gap: 1 }}><DeleteOutlineRoundedIcon /> 削除の確認</DialogTitle>
        <DialogContent sx={{ pt: 1 }}>
          <Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.8)', mb: 2 }}>ファイル「<strong>{selectedFile?.name}</strong>」を削除してもよろしいですか？</Typography>
          <Typography variant="caption" sx={{ color: 'light-dark(rgba(168,6,55,0.8), rgba(250,112,154,0.8))', display: 'block', p: 1.5, bgcolor: 'rgba(250,112,154,0.1)', borderRadius: 2 }}>※この操作は元に戻せません。プロジェクトから完全に削除されます。</Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setDeleteDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', textTransform: 'none' }}>キャンセル</Button>
          <Button onClick={confirmDeleteWorkFile} variant="contained" sx={{ bgcolor: '#fa709a', color: 'var(--brand-fg)', fontWeight: 800, '&:hover': { bgcolor: '#ff90b2' } }}>完全に削除する</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={deleteVersionDialogOpen} onClose={() => setDeleteVersionDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', borderRadius: 3, minWidth: 400 } }}>
        <DialogTitle sx={{ fontWeight: 800, color: 'light-dark(#a80637, #fa709a)', display: 'flex', alignItems: 'center', gap: 1 }}><DeleteOutlineRoundedIcon /> ローカルファイルの削除</DialogTitle>
        <DialogContent sx={{ pt: 1 }}>
          <Typography sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.8)', mb: 2 }}>このローカルファイル版を削除してもよろしいですか？</Typography>
          <Typography variant="caption" sx={{ color: 'light-dark(rgba(168,6,55,0.8), rgba(250,112,154,0.8))', display: 'block', p: 1.5, bgcolor: 'rgba(250,112,154,0.1)', borderRadius: 2 }}>※このファイルはローカルPCから削除されます。元には戻せません。</Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setDeleteVersionDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', textTransform: 'none' }}>キャンセル</Button>
          <Button onClick={confirmDeleteVersion} variant="contained" sx={{ bgcolor: '#fa709a', color: 'var(--brand-fg)', fontWeight: 800, '&:hover': { bgcolor: '#ff90b2' } }}>削除する</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={folderSettingsDialogOpen} onClose={() => setFolderSettingsDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', backgroundImage: 'none' } }}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><FolderOpenRoundedIcon sx={{ color: '#00BFFF' }} />ローカル保存先</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', mb: 3 }}>このプロジェクトのローカルファイルが保存されているフォルダです。</Typography>
          <Box sx={{ p: 2, bgcolor: 'rgb(var(--brand-fg-rgb) / 0.05)', borderRadius: 2, mb: 2 }}>
            <Typography variant="caption" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)', display: 'block', mb: 1 }}>保存先フォルダ</Typography>
            <Typography variant="body2" sx={{ fontFamily: 'monospace', color: 'var(--brand-fg)', wordBreak: 'break-all' }}>{customBaseDir || defaultBaseDir || '—'}</Typography>
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setFolderSettingsDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.5)' }}>閉じる</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={infoDialogType !== null} onClose={() => setInfoDialogType(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', backgroundImage: 'none', borderRadius: 3 } }}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 700, borderBottom: '1px solid rgb(var(--brand-fg-rgb) / 0.1)', pb: 2 }}>
        ...
        <DialogContent sx={{ p: 3, pt: 4 }}>
          {infoDialogType === 'upload' && (
            <>
              <Typography variant="body1" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.9)', mb: 3, lineHeight: 1.6 }}>手元のパソコンに保存されている最新の作業データを、クラウド（Webサーバー）にアップロードして同期します。</Typography>
              <Box sx={{ bgcolor: 'rgba(0,191,255,0.05)', p: 2, borderRadius: 2, border: '1px solid rgba(0,191,255,0.2)' }}>
                ...
                <Typography variant="body2" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', lineHeight: 1.8 }}>・作成した3Dモデルを、クライアントにWeb経由で確認してほしい時<br />・別のパソコンでログインして、作業の続きを行いたい時<br />・プロジェクトのメンバーに最新の進捗を共有したい時</Typography>
              </Box>
            </>
          )}
          {infoDialogType === 'fork' && (
            <>
              <Typography variant="body1" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.9)', mb: 3, lineHeight: 1.6 }}>現在のファイル（履歴とデータ群）を丸ごとコピーし、別の名前をつけて独立した新しいファイルとして分岐させます。</Typography>
              <Box sx={{ bgcolor: 'rgba(0,191,255,0.05)', p: 2, borderRadius: 2, border: '1px solid rgba(0,191,255,0.2)' }}>
                <Typography variant="subtitle2" sx={{ color: '#00BFFF', mb: 1, fontWeight: 700 }}>こんな時に使います：</Typography>
                <Typography variant="body2" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', lineHeight: 1.8 }}>・元のデザイン案（A案）は残しつつ、別のデザイン変更（B案）を試して比較したい時<br />・一つの基本モデルから、複数のカラーバリエーションや仕様違いのモデルを作成したい時</Typography>
              </Box>
            </>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 2 }}><Button onClick={() => setInfoDialogType(null)} variant="outlined" sx={{ color: 'var(--brand-fg)', borderColor: 'rgb(var(--brand-fg-rgb) / 0.3)', '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.1)', borderColor: '#fff' } }}>閉じる</Button></DialogActions>
      </Dialog>

      <Dialog open={editVersionDialogOpen} onClose={() => setEditVersionDialogOpen(false)} PaperProps={{ sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', backgroundImage: 'none', minWidth: 400, borderRadius: 3, border: '1px solid rgb(var(--brand-fg-rgb) / 0.1)' } }}>
        <DialogTitle sx={{ pb: 1, fontWeight: 700 }}>ファイル名の変更</DialogTitle>
        <DialogContent sx={{ p: 3, pt: 1 }}>
          <Typography variant="body2" sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.6)', mb: 2 }}>ローカルのファイル名を変更します。（元の拡張子は自動的に保持されます）</Typography>
          <TextField autoFocus fullWidth label="ファイル名" variant="outlined" value={editVersionName} onChange={(e) => setEditVersionName(e.target.value)} InputLabelProps={{ style: { color: 'rgb(var(--brand-fg-rgb) / 0.5)' } }} InputProps={{ sx: { color: 'var(--brand-fg)', '& .MuiOutlinedInput-notchedOutline': { borderColor: 'rgb(var(--brand-fg-rgb) / 0.2)' }, '&:hover .MuiOutlinedInput-notchedOutline': { borderColor: 'rgb(var(--brand-fg-rgb) / 0.4)' }, '&.Mui-focused .MuiOutlinedInput-notchedOutline': { borderColor: '#00BFFF' } } }} />
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setEditVersionDialogOpen(false)} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.6)', '&:hover': { bgcolor: 'rgb(var(--brand-fg-rgb) / 0.1)' } }}>キャンセル</Button>
          <Button onClick={handleEditVersionSubmit} variant="contained" disabled={!editVersionName.trim()} sx={{ bgcolor: '#00BFFF', color: 'var(--brand-fg)', fontWeight: 700, '&:hover': { bgcolor: '#009acd' } }}>保存</Button>
        </DialogActions>
      </Dialog>
