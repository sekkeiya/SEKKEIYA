...
export const UnsavedOnExitDialog: React.FC<Props> = ({ open, onCancel, onProceedClose }) => {
  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="sm" fullWidth
      slotProps={{ paper: { sx: { bgcolor: 'var(--brand-surface2)', color: 'var(--brand-fg)', border: '1px solid rgb(var(--brand-fg-rgb) / 0.1)' } } }}>
          ...
          {entries.map(([key, info]) => (
            <Box key={key} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, py: 0.75 }}>
              <Checkbox
                ...
                sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.4)', '&.Mui-checked': { color: DIRTY_COLOR }, p: 0.5 }}
              />
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography noWrap sx={{ fontSize: '0.85rem', fontWeight: 600 }}>{info.name}</Typography>
                <Typography noWrap sx={{ fontSize: '0.7rem', color: 'rgb(var(--brand-fg-rgb) / 0.5)' }}>
                  {SCOPE_LABEL[info.scope] || info.scope} ・ {projectName(info.projectId)}{info.isNew ? ' ・ 新規' : ''}
                </Typography>
                {info.isNew && checked[key] && (
                  <Box sx={{ mt: 0.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography sx={{ fontSize: '0.7rem', color: 'rgb(var(--brand-fg-rgb) / 0.5)' }}>公開設定:</Typography>
                    <ToggleButtonGroup
                      size="small" exclusive value={visOf(key)}
                      onChange={(_, v) => v && setVisByKey(prev => ({ ...prev, [key]: v }))}
                      sx={{ '& .MuiToggleButton-root': { color: 'rgb(var(--brand-fg-rgb) / 0.6)', fontSize: '0.68rem', py: 0.1, px: 1, borderColor: 'rgb(var(--brand-fg-rgb) / 0.15)' }, '& .Mui-selected': { color: '#fff !important', bgcolor: 'rgba(255,179,0,0.25) !important' } }}
                    >
      ...
      <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
        <Button onClick={onCancel} disabled={saving} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', textTransform: 'none' }}>
          キャンセル
        </Button>
        <Button onClick={onProceedClose} disabled={saving} sx={{ color: 'rgb(var(--brand-fg-rgb) / 0.7)', textTransform: 'none' }}>
          保存せずに終了
        </Button>
