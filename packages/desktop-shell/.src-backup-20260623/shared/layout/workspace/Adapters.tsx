          ...
          if (targetRefId) {
            try {
              ...
              if (assetSnap.exists()) {
                ...
                return { ...item, ...globalData, id: item.id, status: item.status, resolvedRef: true };
              }
            } catch (e: any) {
  ...
  useEffect(() => {
    ...
    const setupSubscription = async () => {
      ...
      if (scope === 'global_models') {
         ...
         // In SSOT schema, a project's models are local items referencing global master assets
         q = query(collection(db, `projects/${viewingPublicProjectId}/workspaces/models/items`), where('type', '==', 'model'), limit(60));
      } else {
    ...
    const setupSubscription = async () => {
      ...
      unsubscribe = onSnapshot(q as any, (snapshot: any) => {
        const fetchedItems = snapshot.docs.map((doc: any) => {
          ...
          // Provide metadata so the UI recognizes this as a project item
...
const useDssService = (payload?: AdapterContext) => {
  ...
  const projectService = useWorkspaceService(!isGlobal && payload ? { ...payload, appScope: '3dss' } : undefined);
...
const useDsiService = (payload?: AdapterContext) => {
  ...
  const projectService = useDsiLibraryService(!isGlobal && payload ? payload : undefined);
