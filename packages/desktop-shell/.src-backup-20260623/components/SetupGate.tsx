...
import { isTauri } from '@desktop/lib/platform';
...
export function SetupGate({ children }: { children: ReactNode }) {
  // On the web build there is no Blender / native backend — skip the gate entirely.
  const [phase, setPhase] = useState<SetupPhase>(isTauri() ? 'checking' : 'done');
  const [progress, setProgress] = useState<DownloadProgress | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  useEffect(() => {
    if (!isTauri()) return; // web: nothing to set up
