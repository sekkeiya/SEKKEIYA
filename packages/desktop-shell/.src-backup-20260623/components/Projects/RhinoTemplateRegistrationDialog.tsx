...
export const RhinoTemplateRegistrationDialog: React.FC<Props> = ({ open, onClose, onRegister, initialData }) => {
  ...
  const handleSubmit = async () => {
    ...
    const newTemplate: RhinoTemplate = {
      ...
      isPublic: isOfficial || isPublic,
      templatePath: initialData?.templatePath || '',
      isMock: false,
      usageCount: initialData?.usageCount || 0,
      toolType,
      storagePath: initialData?.storagePath,
      thumbnailUrl: initialData?.thumbnailUrl // Carry-over logic
    };
    
    try {
      await onRegister(newTemplate, selectedFile, setStatus, thumbnailFile, glbFile);
    } catch (error) {
      setStatus('error');
    }
  };

  const isFormValid = name.trim() !== '' && (selectedFile !== null || !!initialData) && status !== 'uploading' && status !== 'saving' && status !== 'success';
    ...
    >
      ...
      {status === 'idle' || status === 'error' ? (
        ...
        <DialogContent sx={{ px: 3, pb: 6, pt: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
          ...
          <Typography variant="h6" fontWeight={700}>
            {status === 'uploading' && '3dmファイルをアップロード中...'}
      ...
      {status === 'idle' || status === 'error' ? (
        <DialogActions sx={{ px: 3, py: 2, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <Button onClick={onClose} sx={{ color: 'rgba(255,255,255,0.5)' }}>キャンセル</Button>
          <Button onClick={handleSubmit} disabled={!isFormValid} variant="contained" sx={{ bgcolor: '#90caf9', color: '#000', '&:hover': { bgcolor: '#64b5f6' }, '&.Mui-disabled': { bgcolor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.3)' } }}>
