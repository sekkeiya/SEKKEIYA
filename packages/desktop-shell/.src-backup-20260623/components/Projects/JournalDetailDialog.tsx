...
export const JournalDetailDialog: React.FC<JournalDetailDialogProps> = ({ 
  ...
  const renderContextChips = () => {
    ...
    const tooltipContent = (
      <Box sx={{ p: 0.5, maxWidth: 300 }}>
        ...
        <Typography variant="caption" sx={{ display: 'block', mt: 1.5, color: '#9aa0a6', fontSize: '0.65rem' }}>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: '#202124', color: '#e8eaed' } }}>
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        {isEditing ? (
          <InputBase
            ...
            sx={{ color: '#fff', fontSize: 20, fontWeight: 'bold', flex: 1, mr: 2 }}
          />
        ) : (
        ...
        <Box>
          {!isEditing && isAuthor && (
            <>
              <IconButton size="small" onClick={() => setIsEditing(true)} sx={{ color: '#8ab4f8', mr: 1 }}>
                <EditIcon fontSize="small" />
              </IconButton>
              <IconButton size="small" onClick={handleDelete} sx={{ color: '#f28b82', mr: 1 }}>
      ...
      <DialogContent dividers sx={{ borderColor: 'rgba(255,255,255,0.1)' }}>
        ...
        {isEditing ? (
          ...
          <Box sx={{ 
            ...
            '& pre': { bgcolor: 'rgba(0,0,0,0.3)', p: 2, borderRadius: 1, overflowX: 'auto' },
            '& a': { color: '#8ab4f8' },
            '& blockquote': { borderLeft: '4px solid #5f6368', pl: 2, ml: 0, color: '#9aa0a6' },
      ...
      {isEditing && (
        <DialogActions sx={{ borderTop: '1px solid rgba(255,255,255,0.1)', p: 2, display: 'flex', justifyContent: 'space-between' }}>
          <Box>
            <input 
              type="file" 
              accept="image/*" 
            ...
            />
            <IconButton sx={{ color: '#9aa0a6' }} size="small" onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
              <ImageIcon />
            </IconButton>
          </Box>
          <Box>
            <Button onClick={() => setIsEditing(false)} sx={{ color: '#9aa0a6', mr: 1 }}>キャンセル</Button>
