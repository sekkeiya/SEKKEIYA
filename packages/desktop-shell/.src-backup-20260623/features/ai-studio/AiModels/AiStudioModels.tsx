...
export const AiStudioModels: React.FC = () => {
  ...
  const [selectedId, setSelectedId] = useState<string>(aiProfiles[0]?.id || '');
  ...
  const renderCategoryList = (category: AiProfile['category'], title: string, icon: React.ReactNode) => {
    ...
    return (
      <Box sx={{ mb: 3 }}>
        <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, fontWeight: 700, px: 2, mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
          {icon} {title}
        </Typography>
        <List disablePadding>
          {models.map(model => (
             <ListItem key={model.id} disablePadding sx={{ mb: 0.5 }}>
               <ListItemButton
                 ...
                 sx={{
                   mx: 1,
                   borderRadius: 2,
                   bgcolor: selectedId === model.id ? 'rgba(255,255,255,0.1)' : 'transparent',
                   border: selectedId === model.id 
                      ? (model.category === 'Orchestrator' ? `1px solid ${'#a855f7'}` : '1px solid rgba(255,255,255,0.2)') 
               ...
               >
                 <ListItemText 
                   ...
                   secondaryTypographyProps={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}
                 />
                 {model.status === 'Active' && (
  ...
  return (
    <Box sx={{ display: 'flex', width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      <Box sx={{ 
        ...
        <Typography variant="h6" sx={{ color: '#fff', px: 2, mb: 3, fontWeight: 700 }}>AI モデル管理</Typography>
                   ...
                   <ListItem key={model.id} disablePadding sx={{ mb: 0.5 }}>
                     <ListItemButton
                       ...
                       sx={{
                         ...
                         borderRadius: 2,
                         bgcolor: selectedId === model.id ? 'rgba(255,255,255,0.1)' : 'transparent',
                         border: selectedId === model.id ? '1px solid rgba(255,255,255,0.2)' : '1px solid transparent',
                         '&:hover': { bgcolor: 'rgba(255,255,255,0.05)' }
                       }}
                     >
                       <ListItemText 
                         primary={model.name}
                         secondary={model.id}
                         primaryTypographyProps={{ fontSize: 13, fontWeight: selectedId === model.id ? 600 : 500, color: selectedId === model.id ? '#fff' : 'rgba(255,255,255,0.7)' }}
                         secondaryTypographyProps={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}
                       />
      ...
      <Box sx={{ flex: 1, p: { xs: 2, md: 3, lg: 4 }, overflowY: 'auto', position: 'relative' }}>
        <AnimatePresence mode="wait">
          ...
          <Box sx={{ mb: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1, gap: 2 }}>
              ...
              {selectedModel.category === 'Orchestrator' && (
                 <Chip label="CORE" size="small" sx={{ bgcolor: '#a855f7', color: '#fff', fontWeight: 'bold' }} />
              )}
            </Box>
            ...
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
               ...
               <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', bgcolor: 'rgba(255,255,255,0.05)', px: 1, py: 0.5, borderRadius: 1 }}>
                 Role: {selectedModel.role}
               </Typography>
               {selectedModel.usageScopes.map(scope => (
                 <Typography key={scope} variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', bgcolor: 'rgba(255,255,255,0.05)', px: 1, py: 0.5, borderRadius: 1 }}>
                   Scope: {scope}
                 </Typography>
               ))}
            ...
            return (
              <Tabs 
                ...
                sx={{ 
                  ...
                  '& .MuiTab-root': { color: 'rgba(255,255,255,0.5)', textTransform: 'none', minWidth: 100, fontWeight: 600 },
          ...
          <Paper sx={{ p: 4, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)', mb: 4, minHeight: 400 }}>
            {(() => {
              const meta = AI_MODEL_METADATA[selectedModel.id];
              return (
                <>
                  ...
                  {meta && activeTab === 0 && (
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1, fontSize: 16 }}>モデルの役割と目的</Typography>
                      <Typography sx={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, mb: 4, lineHeight: 1.6 }}>
                        {meta.overview.purpose}
                      </Typography>

                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>このモデルが解決する主な問い</Typography>
                      <List sx={{ mb: 4 }}>
                        {meta.overview.questions.map((q, i) => (
                          <ListItem key={i} sx={{ px: 0, py: 0.5 }}>
                            <Box sx={{ mr: 1.5, fontSize: 18 }}>{q.icon}</Box>
                            <ListItemText primary={q.text} primaryTypographyProps={{ color: 'rgba(255,255,255,0.8)', fontSize: 14 }} />
                          </ListItem>
                        ))}
                      </List>

                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>入出力フロー概要</Typography>
                      <Typography sx={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, mb: 3 }}>
                        {meta.overview.flow}
                      </Typography>

                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>期待される出力形式 (JSON Schema 等)</Typography>
                      <Paper sx={{ p: 2, bgcolor: 'rgba(0,0,0,0.4)', borderRadius: 2 }}>
                        <Typography sx={{ color: '#a855f7', fontSize: 13, fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
                  ...
                  {meta && activeTab === 2 && (
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 3 }}>コストと使用状況のモニタリング</Typography>
                      <Stack direction="row" spacing={3} sx={{ mb: 4 }}>
                        <Paper sx={{ flex: 1, p: 3, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)' }}>
                          <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, mb: 1 }}>今月の呼び出し回数</Typography>
                          <Typography sx={{ color: '#fff', fontSize: 28, fontWeight: 700 }}>
                            {meta.cost.currentUsage.toLocaleString()} <Typography component="span" sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>/ {meta.cost.monthlyLimit > 0 ? meta.cost.monthlyLimit.toLocaleString() : '∞'}</Typography>
                          </Typography>
                        </Paper>
                        <Paper sx={{ flex: 1, p: 3, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)' }}>
                          <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, mb: 1 }}>今月の推計コスト</Typography>
                          <Typography sx={{ color: '#a855f7', fontSize: 28, fontWeight: 700 }}>
                      ...
                      {meta.cost.monthlyLimit > 0 && (
                        <Box sx={{ mb: 4 }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            <Typography sx={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>月間リミットへの到達率</Typography>
                            <Typography sx={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{((meta.cost.currentUsage / meta.cost.monthlyLimit) * 100).toFixed(1)}%</Typography>
                          </Box>
                      ...
                      <Alert icon={false} sx={{ bgcolor: 'rgba(59,130,246,0.1)', color: '#60a5fa', border: '1px solid rgba(59,130,246,0.2)' }}>
                  ...
                  {meta && activeTab === 3 && (
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 2 }}>学習ベースとデータの扱い</Typography>
                      <Paper sx={{ p: 0, bgcolor: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 2, overflow: 'hidden', mb: 4 }}>
                        <List disablePadding>
                          <ListItem sx={{ borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}>
                            <ListItemText primary="システム基礎データ" secondary={meta.data.baseData} primaryTypographyProps={{ color: '#fff', fontSize: 14 }} secondaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', mt: 0.5 }} />
                          </ListItem>
                          <ListItem sx={{ borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}>
                            <ListItemText primary="継続的な Fine-tuning" secondary={meta.data.finetuning ? '本環境で自動実行される' : '実施しない'} primaryTypographyProps={{ color: '#fff', fontSize: 14 }} secondaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', mt: 0.5 }} />
                            {!meta.data.finetuning && <Chip label="なし" size="small" sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)' }} />}
                          </ListItem>
                          <ListItem sx={{ borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}>
                            <ListItemText primary="チャット・操作履歴の学習" secondary={meta.data.learnsFromHistory ? '会話文脈およびメタデータを学習用コーパスへマージ' : '会話からの自動学習は行わない'} primaryTypographyProps={{ color: '#fff', fontSize: 14 }} secondaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', mt: 0.5 }} />
                          </ListItem>
                          <ListItem sx={{ borderBottom: '1px solid rgba(255,255,255,0.05)', py: 2 }}>
                            <ListItemText primary="プロジェクト参照スコープ" secondary={meta.data.projectDataScope} primaryTypographyProps={{ color: '#fff', fontSize: 14 }} secondaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', mt: 0.5 }} />
                          </ListItem>
                          <ListItem sx={{ py: 2 }}>
                            <ListItemText primary="データの外部送信先" secondary={meta.data.externalDestination} primaryTypographyProps={{ color: '#fff', fontSize: 14 }} secondaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', mt: 0.5 }} />
                          </ListItem>
                        </List>
                      </Paper>
                      
                      {!meta.data.learnsFromHistory && (
                        <Alert severity="info" align="left" sx={{ bgcolor: 'rgba(168,85,247,0.1)', color: '#d8b4fe', border: '1px solid rgba(168,85,247,0.2)' }}>
                  ...
                  {meta && activeTab === 4 && (
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>このAIはいつ呼び出されるか？</Typography>
                      <List sx={{ mb: 3 }}>
                        {meta.trigger.calledCases.map((c, i) => (
                           <ListItem key={i} sx={{ px: 0, py: 1 }}>
                             ...
                             <ListItemText 
                               primary={c.text} 
                               secondary={<Box component="span" sx={{ fontFamily: 'monospace', display: 'inline-block', mt: 0.5, px: 0.5, bgcolor: 'rgba(255,255,255,0.1)', borderRadius: 1 }}>{c.codePath}</Box>} 
                      ...
                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>呼び出されないケース</Typography>
                      <List sx={{ mb: 3 }}>
                        {meta.trigger.notCalledCases.map((txt, i) => (
                           <ListItem key={i} sx={{ px: 1, py: 0.5 }}>
                             <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.3)', mr: 2 }} />
                             <ListItemText primary={txt} primaryTypographyProps={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }} />
                           </ListItem>
                        ))}
                      </List>

                      <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>データフロー (Data Flow)</Typography>
                      <Paper sx={{ p: 2, bgcolor: 'rgba(0,0,0,0.4)', borderRadius: 2 }}>
                  ...
                  {((meta && activeTab === 1) || (!meta && activeTab === 0)) && (
                    <Box>
                      ...
                      <Box sx={{ mb: 5 }}>
                        <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>推論エンジン (Base Model)</Typography>
                        {meta ? (
                           <Box sx={{ p: 2, bgcolor: 'rgba(0,0,0,0.3)', borderRadius: 2, border: '1px solid rgba(255,255,255,0.05)', mt: 1 }}>
                               ...
                               {meta.engine.switchable ? (
                                 <FormControl size="small" sx={{ minWidth: 240 }}>
                                   <Select
                                     ...
                                     sx={{ color: '#a855f7', fontWeight: 'bold', '& .MuiOutlinedInput-notchedOutline': { borderColor: 'rgba(255,255,255,0.2)' } }}
                                   >
                                           ...
                                           <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', justifyContent: 'space-between', gap: 2 }}>
                                             ...
                                             {!isAllowed && (
                                               <Chip 
                                                 ...
                                                 size="small" 
                                                 sx={{ height: 20, fontSize: 10, bgcolor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', pl: 0.5 }} 
                                               />
                                             )}
                                           </Box>
                                         </MenuItem>
                                 ...
                                 <Typography sx={{ color: '#a855f7', fontWeight: 'bold' }}>{meta.engine.baseModel}</Typography>
                               )}
                               {!meta.engine.switchable && <Chip label="変更不可" size="small" sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', fontWeight: 600 }} />}
                             </Box>
                             <Divider sx={{ borderColor: 'rgba(255,255,255,0.1)', mb: 1.5 }} />
                             <Stack spacing={1}>
                               <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}><strong>呼び出し方式:</strong> <span style={{ fontFamily: 'monospace', color: '#e9d5ff', marginLeft: 8 }}>{meta.engine.callMethod}</span></Typography>
                               <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}><strong>レスポンス形式:</strong> <span style={{ fontFamily: 'monospace', color: '#e9d5ff', marginLeft: 8 }}>{meta.engine.responseFormat}</span></Typography>
                               <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}><strong>プロンプト管理:</strong> <span style={{ marginLeft: 8 }}>{meta.engine.promptLocation}</span></Typography>
                               <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}><strong>平均アクセス:</strong> <span style={{ marginLeft: 8 }}>{meta.engine.avgResponseTime}</span></Typography>
                             </Stack>
                           </Box>
                        ) : (
                          <>
                            ...
                            <FormControl fullWidth size="small" sx={{ mb: 1 }}>
                              <Select
                                ...
                                sx={{ 
                                  bgcolor: 'rgba(0,0,0,0.3)', color: tempBaseModelId === 'hybrid-rule-engine' ? 'rgba(255,255,255,0.5)' : '#fff',
                      ...
                      {/* Usage Scopes Editor */}
                      <Box sx={{ mb: 5 }}>
                        <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>利用スコープ (Usage Scopes)</Typography>
                        <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, mb: 2 }}>
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                          {AVAILABLE_SCOPES.map(scope => (
                            <Chip
                              ...
                              sx={{
                                bgcolor: tempUsageScopes.includes(scope) ? 'rgba(168,85,247,0.3)' : 'rgba(255,255,255,0.05)',
                                color: tempUsageScopes.includes(scope) ? '#e9d5ff' : 'rgba(255,255,255,0.4)',
                                border: `1px solid ${tempUsageScopes.includes(scope) ? '#a855f7' : 'transparent'}`,
                                '&:hover': { bgcolor: tempUsageScopes.includes(scope) ? 'rgba(168,85,247,0.4)' : 'rgba(255,255,255,0.1)' }
                              }}
                            />
                          ))}
                        </Stack>
                  ...
                  {!meta && activeTab === 1 && !['hybrid-rule-engine'].includes(tempBaseModelId) && !selectedModel.id.includes('gemini') && (
                    <Box>
                      ...
                      <Box sx={{ mb: 5 }}>
                        <Typography sx={{ color: '#fff', fontWeight: 600, mb: 1 }}>接続中の Knowledge & RAG</Typography>
                        <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, mb: 2 }}>
                        ...
                        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
                           <Chip 
                             ...
                             sx={{ bgcolor: 'rgba(255,255,255,0.08)', color: '#fff', '&:hover': { bgcolor: 'rgba(255,255,255,0.15)' } }}
                        ...
                        <Menu
                          ...
                          PaperProps={{ sx: { bgcolor: '#1a1f2c', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', minWidth: 280 } }}
                        >
                          {currentKnowledge.length === 0 ? (
                            <MenuItem disabled sx={{ color: 'rgba(255,255,255,0.5)' }}>データソースがありません</MenuItem>
                          ) : (
                      ...
                      <Box sx={{ mb: 5, p: 2, borderRadius: 2, bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <Box>
                            <Typography sx={{ color: '#fff', fontWeight: 600 }}>Save Data (Derived Memory) の参照</Typography>
                        ...
                        {tempUseSaveData && (
                          <Box sx={{ mt: 3, pt: 3, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                            <Typography sx={{ color: '#fff', fontWeight: 600, mb: 2 }}>Memory Synthesis Pipeline</Typography>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
                              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
                                未要約の操作記録 (Raw Events): <span style={{ color: '#a855f7', fontWeight: 'bold' }}>{unsummarizedEvents.length}</span> 件
                              </Typography>
                              <Button 
                                variant="outlined" 
                                size="small" 
                                ...
                                sx={{ color: '#a855f7', borderColor: 'rgba(168,85,247,0.5)', textTransform: 'none', '&:hover': { borderColor: '#a855f7', bgcolor: 'rgba(168,85,247,0.1)' } }}
                              >
                            ...
                            {myMemories.length > 0 ? (
                              <Box sx={{ mt: 2 }}>
                                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.5)', mb: 1, display: 'block' }}>生成済みの Memory Fragments</Typography>
                                <Stack spacing={1}>
                                  {myMemories.map(m => (
                                    <Paper key={m.id} sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 2 }}>
                                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                         <Typography variant="body2" sx={{ fontWeight: 600, color: '#e9d5ff' }}>[{m.topic}]</Typography>
                                         <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', bgcolor: 'rgba(255,255,255,0.05)', px: 0.5, borderRadius: 1 }}>
                              ...
                              <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.3)' }}>Memoriesはまだありません。イベントが溜まったら要約を実行してください。</Typography>
                            )}
                          </Box>
                        )}
                      </Box>
                  ...
                  {(!meta && activeTab === 2) && (
                    <Box>
                      ...
                      {!['hybrid-rule-engine'].includes(tempBaseModelId) && !selectedModel.id.includes('gemini') && (
                        <Box sx={{ mb: 5 }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            <Typography sx={{ color: '#fff', fontWeight: 600 }}>Temperature (創造性 / ランダム性)</Typography>
                            <Typography sx={{ color: '#a855f7', fontWeight: 700 }}>{tempTemperature.toFixed(1)}</Typography>
                          </Box>
                          <Slider
                            min={0}
                            max={1.5}
                          ...
                          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>0.0 (厳密・正確)</Typography>
                            <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>1.5 (独創的)</Typography>
                          </Box>
                        </Box>
                      )}
                      ...
                      {!selectedModel.id.includes('gemini') ? (
                        <Box>
                           ...
                           <TextField
                             ...
                             InputProps={{ sx: { color: tempBaseModelId === 'hybrid-rule-engine' ? 'rgba(255,255,255,0.6)' : '#fff', fontFamily: 'monospace', fontSize: 14 } }}
                             sx={{ 
                               bgcolor: 'rgba(0,0,0,0.3)',
                               borderRadius: 2,
                               '& .MuiOutlinedInput-root': {
                                 '& fieldset': { borderColor: 'rgba(255,255,255,0.1)' },
                                 '&:hover fieldset': { borderColor: tempBaseModelId === 'hybrid-rule-engine' ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.2)' },
                                 '&.Mui-focused fieldset': { borderColor: tempBaseModelId === 'hybrid-rule-engine' ? 'rgba(255,255,255,0.1)' : '#a855f7' },
                               }
                             }}
                           />
                        </Box>
                      ...
                      {tempBaseModelId === 'hybrid-rule-engine' && (
                        <Box sx={{ mt: 4, pt: 3, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                          ...
                          <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, mb: 2 }}>
                            ユーザーがS.Models上でメタデータ（サイズ、カテゴリ等）を修正した履歴です。<br/>
                            これらのデータは、自動分類AIがルールベースと組み合わせて学習するための教師データとして蓄積されます。
                          </Typography>
                          
                          <Paper sx={{ p: 2, bgcolor: 'rgba(0,0,0,0.3)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Box>
                              <Typography sx={{ color: '#fff', fontSize: 14 }}>
                                蓄積されたフィードバック件数
                              </Typography>
                              <Typography sx={{ color: '#a855f7', fontSize: 24, fontWeight: 700 }}>
                                {correctionEvents.length} <Typography component="span" sx={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>件</Typography>
                              </Typography>
                            </Box>
                            <Button 
                              variant="contained" 
                              ...
                              sx={{ bgcolor: 'rgba(168, 85, 247, 0.2)', color: '#d8b4fe', '&:hover': { bgcolor: 'rgba(168, 85, 247, 0.3)' }, '&.Mui-disabled': { bgcolor: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.2)' } }}
                            >
                              データを確認
                            </Button>
                          </Paper>
           ...
           <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
              ...
              <Button 
                ...
                sx={{ 
                  ...
                  '&.Mui-disabled': { bgcolor: 'rgba(168,85,247,0.3)', color: 'rgba(255,255,255,0.3)' }
                }}
              >
      ...
      >
        <Alert onClose={() => setSaveToast(false)} severity="success" sx={{ width: '100%', bgcolor: '#1a1f2c', color: '#4ade80', border: '1px solid #4ade80' }}>
          プロファイルを保存しました
        </Alert>
      </Snackbar>
      ...
      <Dialog 
        ...
        PaperProps={{ sx: { bgcolor: '#1e1e1e', color: '#fff', width: 600, maxWidth: '100%', borderRadius: 2 } }}
      >
        <DialogTitle sx={{ borderBottom: '1px solid rgba(255,255,255,0.1)', pb: 2, fontWeight: 600 }}>
          学習元データ (ユーザーフィードバック)
        </DialogTitle>
        <DialogContent sx={{ mt: 2, p: 3 }}>
          {correctionEvents.length === 0 ? (
             <Typography sx={{ color: 'rgba(255,255,255,0.5)', textAlign: 'center', py: 4 }}>
               まだフィードバックデータがありません。S.Modelsでメタデータを修正すると記録されます。
             </Typography>
          ) : (
            <List disablePadding>
              {correctionEvents.map((ev, index) => (
                <Paper key={ev.id} sx={{ mb: 2, p: 2, bgcolor: 'rgba(0,0,0,0.3)', borderRadius: 2 }}>
                   <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1, alignItems: 'center' }}>
                     <Typography sx={{ fontSize: 13, color: '#a855f7', fontWeight: 600 }}>
        ...
        <DialogActions sx={{ borderTop: '1px solid rgba(255,255,255,0.1)', p: 2 }}>
          <Button onClick={() => setTrainingDataOpen(false)} sx={{ color: 'rgba(255,255,255,0.7)', textTransform: 'none', fontWeight: 600 }}>
            閉じる
          </Button>
        </DialogActions>
      </Dialog>
