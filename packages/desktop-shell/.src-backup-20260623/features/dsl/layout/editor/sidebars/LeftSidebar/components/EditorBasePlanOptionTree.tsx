...
export default function EditorBasePlanOptionTree() {
  ...
  const handleSelectBase = useCallback(
    (baseId: string) => {
      if (baseId && baseId !== selectedBaseId) selectBase(baseId);
    },
    ...
    <Box sx={{ px: 1.5, py: 1, height: "100%", overflowY: "auto", overflowX: "hidden" }}>
      <Stack spacing={0.5}>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <ListItemButton 
            onClick={() => setIsProjectTabOpen(s => !s)} 
            disableRipple
            sx={{ p: 0, '&:hover': { bgcolor: 'transparent' }, width: 'auto', flexGrow: 0 }}
          >
            {isProjectTabOpen ? (
              <ExpandMoreRoundedIcon fontSize="small" sx={{ mr: 0.5, opacity: 0.8 }} />
            ) : (
              <ChevronRightRoundedIcon fontSize="small" sx={{ mr: 0.5, opacity: 0.8 }} />
            )}
            <Typography variant="subtitle2" sx={{ opacity: 0.9, fontWeight: 700, fontSize: '0.8rem', lineHeight: 1.2 }}>
              Project
            </Typography>
          </ListItemButton>
        ...
        <Collapse in={isProjectTabOpen} timeout="auto" unmountOnExit>
          {safeBases.length === 0 ? (
            ...
            {safeBases.map((b) => {
              ...
              return (
                <Box key={b.id} sx={{ borderBottom: `1px solid ${border}`, pb: 0.25, mb: 0.25 }}>
                  ...
                  {/* === Plans (only shown for the selected base) === */}
                  {openBase && (
