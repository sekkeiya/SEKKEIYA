...
function getAssetCategory(asset: any): string {
  ...
  if (asset.extendedMetadata?.mainCategory) return asset.extendedMetadata.mainCategory.toLowerCase();
  if (asset.metadata?.category) return asset.metadata.category.toLowerCase();
  if (asset.category) return asset.category.toLowerCase();
  
  const t = (asset.title || asset.metadata?.title || asset.name || '').toLowerCase();
  if (t.includes('sofa') || t.includes('ソファ')) return 'sofa';
  if (t.includes('chair') || t.includes('チェア') || t.includes('椅子')) return 'chair';
  if (t.includes('table') || t.includes('desk') || t.includes('テーブル') || t.includes('机')) return 'table';
  if (t.includes('bed') || t.includes('ベッド')) return 'bed';
  if (t.includes('plant') || t.includes('植物') || t.includes('観葉植物')) return 'plant';
...
export const FurnitureSwapDialog: React.FC<FurnitureSwapDialogProps> = ({
  ...
  const handleApply = () => {
    ...
    const newItems = placedItems.map(item => {
      ...
      const title = newAsset.metadata?.title || newAsset.metadata?.name || newAsset.title || newAsset.name || 'Item';
      const brand = newAsset.metadata?.brand || newAsset.metadata?.brandName || '';
      const thumbUrl = newAsset.metadata?.thumbnail || newAsset.metadata?.thumbnailUrl || newAsset.thumbnailUrl || newAsset.thumbUrl || newAsset.coverUrl || null;
      const glbUrl = newAsset.metadata?.glbUrl || newAsset.metadata?.downloadUrl || newAsset.glbUrl || newAsset.modelUrl || null;
      ...
      return {
        ...
        thumbUrl,
        glbUrl,
        itemRef: newAsset.itemRef || `assets/${newAsset.id}`,
        snapshot: {
          title,
          brand,
