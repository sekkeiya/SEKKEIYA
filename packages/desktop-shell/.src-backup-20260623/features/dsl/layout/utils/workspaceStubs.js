...
export const deleteStructureCascade = async (projectId, workspaceId, nodeId, planType) => {
    ...
    if (planType === "base") {
        descendantIds = all
            .filter((d) => d.id !== nodeId && d.rootBaseId === nodeId)
            .map((d) => d.id);
...
export const deleteLayout = async (projectId, workspaceId, layoutId) => {
    ...
    if (layoutSnap.exists()) {
        ...
        if (urlsToDelete.length > 0) {
            try {
                const { storage } = await import("@desktop/lib/firebase/client");
                const { ref: storageRef, deleteObject } = await import("firebase/storage");
                
                for (const urlOrPath of urlsToDelete) {
                    try {
                        // Firebase storage ref() can handle both paths and full HTTP download URLs
                        await deleteObject(storageRef(storage, urlOrPath));
                        console.log(`Successfully deleted storage object: ${urlOrPath}`);
                    } catch (err) {
                        // Ignore object-not-found errors as it might have been deleted already
                        if (err?.code !== 'storage/object-not-found') {
                            console.warn("Failed to delete object from storage", urlOrPath, err);
                        }
                    }
                }
            } catch (err) {
