...
import { useDsdStore, type EnvEditorTab } from './store/useDsdStore';
import { EnvDiagramCanvas, type EnvDiagramCanvasHandle } from './canvas/EnvDiagramCanvas';
import { BRAND } from '../../styles/theme';
import { save }      from '@tauri-apps/plugin-dialog';
import { writeFile } from '@tauri-apps/plugin-fs';

const ACCENT = '#80cbc4';
const TITLE  = '環境ダイアグラム';
...
const DOCK_TABS: { key: EnvEditorTab; icon: React.ReactNode; label: string }[] = [
  { key: 'building', icon: <HomeWorkRoundedIcon sx={{ fontSize: 18 }} />, label: '建物' },
  { key: 'env',      icon: <AirRoundedIcon      sx={{ fontSize: 18 }} />, label: '環境' },
...
export const EnvironmentDiagramEditor: React.FC = () => {
  ...
  const canvasRef   = useRef<EnvDiagramCanvasHandle>(null);
  ...
  const { style, setStyle, envEditorTab, setEnvEditorTab } = useDsdStore();
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[EnvironmentDiagramEditor] Thumbnail upload failed (non-fatal)', thumbErr);
      }
      useDsdStore.getState().bumpSavedTick(); // 未保存ドット解除トリガ
      setSaveStatus('saved');
      setTimeout(() => setSaveStatus('idle'), 2500);
    } catch (e) {
      console.error('[EnvironmentDiagramEditor] Save failed', e);
      setSaveStatus('error');
  ...
  const handleExportGif = useCallback(async (w: number, h: number) => {
    ...
    try {
      ...
      for (let i = 0; i < FRAMES; i++) {
        // t advances at 1.0/sec — matches live animation speed
        const t = i * GIF_DELAY_MS / 1000;
        const imgData = handle.renderFrame(w, h, t);
        ...
        // Yield to event loop every 10 frames so progress renders
        if (i % 10 === 9) {
      ...
      try {
        const path = await save({
          defaultPath: `${TITLE}_${gifDurationSec}s_${w}x${h}.gif`,
        ...
        const a    = document.createElement('a');
        a.href = url; a.download = `${TITLE}_${gifDurationSec}s_${w}x${h}.gif`; a.click();
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default', overflow: 'hidden' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ position: 'relative' }}>
          <Button
            size="small"
            variant="contained"
            startIcon={isGeneratingGif
              ? <CircularProgress size={12} sx={{ color: '#003c3c' }} />
              : <DownloadRoundedIcon fontSize="small" />}
            onClick={() => !isGeneratingGif && setExportMenuOpen(!exportMenuOpen)}
            disabled={isGeneratingGif}
            sx={{
              bgcolor: ACCENT, color: '#003c3c',
              '&:hover': { bgcolor: '#a7ffeb' },
              fontSize: '0.75rem', fontWeight: 700, textTransform: 'none',
              '&.Mui-disabled': { bgcolor: 'rgba(128,203,196,0.4)', color: '#003c3c' },
            }}
          >
      ...
      >
        <EnvDiagramCanvas ref={canvasRef} width={canvasSize.w} height={canvasSize.h} />
