...
import GlobalProjectDashboard from './pages/GlobalProjectDashboard';
import GlobalAppHub from './pages/GlobalAppHub';
  ...
  return (
    <Routes>
      <Route path="/*" element={
        ...
        currentMainView === 'app-hub' ? <GlobalAppHub /> :
...
export const GlobalModals = () => {
    ...
    (async () => {
      try {
        ...
        unlisten = await win.onCloseRequested((event) => {
          ...
          if (!hasUnsaved) return; // 未保存なし → 通常終了
  ...
  useEffect(() => {
    if (!isTauri()) return; // web: OS通知アクションは無し
    (async () => {
      try {
  ...
  useEffect(() => {
    if (!isTauri()) return; // web: OS通知リスナーは無し
    ...
    (async () => {
      // 1) Windows: tauri-winrt-notification 経由のイベント
      try {
      ...
      // 2) フォールバック: Tauri 通知プラグインの onAction
      try {
