...
export const MiniSidebar = ({ isExpanded, onToggle = () => {} }) => {
  ...
  // 3DSC-specific mock boards or fetch from a hook when implemented
  ...
  return (
    <GlobalMiniSidebar
      currentApp="create"
      currentBoardId={null} // 3DSC routing has no board ID yet
      boards={boards}
      user={null}
      ...
      onToggle={onToggle}
      onLogout={() => window.location.assign('/login?return_to=/app/create/')}
    />
  );
};
