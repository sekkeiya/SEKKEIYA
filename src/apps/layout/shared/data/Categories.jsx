 ...
 *
 * 重要:
 * - Firestore の既存データ互換のため「旧ラベル」もマッピングで保持する
 * - URL slug は既存互換（overall/parts/outside）を維持する
 */
...
export const SUBTYPES = {
  ...
  // ✅ UI表示は案A
  [TYPES.ARCHITECTURE]: ["建具・部材", "建物（本体）", "外構・周辺"],
};

/** =========================
 *  互換：旧サブタイプ名 → 新サブタイプ名
 *  ========================= */
export const ARCH_SUBTYPE_LEGACY_MAP = {
  全体: "建物（本体）",
  パーツ: "建具・部材",
  外構: "外構・周辺",
};
...
/** =========================
 *  互換：新サブタイプ名 → 旧サブタイプ名（既存slugのため）
 *  ========================= */
export const ARCH_SUBTYPE_CANONICAL_TO_LEGACY = {
  "建物（本体）": "全体",
  "建具・部材": "パーツ",
  "外構・周辺": "外構",
};
...
/** =========================
 *  サブタイプ正規化（旧/新/スラッグ混在を吸収）
 *  ========================= */
export function normalizeArchitectureSubTypeLabel(subTypeLabelOrSlug) {
  const v = String(subTypeLabelOrSlug || "").trim();
  if (!v) return "建物（本体）";

  // まず slug -> 旧ラベルへ
  const fromSlug = SUBTYPE_FROM_SLUG?.[TYPES.ARCHITECTURE]?.[v];
  const maybeOld = fromSlug || v;

  // 旧ラベルなら新に寄せる
  if (ARCH_SUBTYPE_LEGACY_MAP[maybeOld]) return ARCH_SUBTYPE_LEGACY_MAP[maybeOld];

  // すでに新ラベルならそのまま
  if (SUBTYPES[TYPES.ARCHITECTURE].includes(maybeOld)) return maybeOld;
...
export const TAXONOMY = {
  ...
  [TYPES.ARCHITECTURE]: {
    ...
    datasets: {
      // ✅ 新ラベルで持つ（UI/アプリ側の標準）
      "建物（本体）": categoryOptionsArchitecture,
      "建具・部材": categoryOptionsArchitectureParts,
      "外構・周辺": categoryOptionsArchitectureOutside,

      // ✅ 旧ラベル互換（古いコードや既存値でも落ちない）
...
export const getCategoryTree = (type, subType = "default") => {
  ...
  // 建築は subType が旧/新/slug でもOKにする
  if (type === TYPES.ARCHITECTURE) {
    const normalized = normalizeArchitectureSubTypeLabel(subType);
    return node.datasets?.[normalized] ?? node.datasets?.[subType] ?? {};
  }
...
export const GROUP_SLUGS = {
  ...
  // ✅ slugキーは既存の overall/parts/outside を維持
...
export const getGroupSlug = (type, subTypeLabel, groupKey) => {
  if (type === TYPES.ARCHITECTURE) {
    const normalized = normalizeArchitectureSubTypeLabel(subTypeLabel);
    const stSlug = getSubtypeSlug(type, normalized); // overall/parts/outside
...
// ✅ 逆引きを一度だけ生成（O(1)）
const SUBITEM_FROM_SLUG = Object.fromEntries(
  Object.entries(SUBITEM_SLUGS).map(([label, slug]) => [slug, label])
);

// ラベル -> スラッグ
export const getSubSlug = (label) => SUBITEM_SLUGS[label] ?? encodeURIComponent(label ?? "");

// スラッグ -> ラベル
export const getSubLabelFromSlug = (slug) => {
  const s = String(slug ?? "");
  return SUBITEM_FROM_SLUG[s] ?? decodeURIComponent(s);
};
...
export function buildCategoryPath(
  ...
  if (t === TYPES.ARCHITECTURE && subTypeLabel) {
    const normalized = normalizeArchitectureSubTypeLabel(subTypeLabel);
    parts.push(getSubtypeSlug(t, normalized));
  }
...
export function parseCategoryParams(type, { subType, group, leaf } = {}) {
  ...
  const subTypeLabel =
    t === TYPES.ARCHITECTURE && subType ? getSubtypeLabelFromSlug(t, subType) : undefined;

  const normalizedSubTypeLabel =
    t === TYPES.ARCHITECTURE ? normalizeArchitectureSubTypeLabel(subTypeLabel) : undefined;

  const groupLabel = group
    ? getGroupLabelFromSlug(t, normalizedSubTypeLabel ?? subTypeLabel, group)
    : undefined;

  const subLabel = leaf ? getSubLabelFromSlug(leaf) : undefined;

  return { type: t, subTypeLabel: normalizedSubTypeLabel ?? subTypeLabel, groupLabel, subLabel };
}
...
export function isSameSubType(type, a, b) {
  ...
  const na = t === TYPES.ARCHITECTURE ? normalizeArchitectureSubTypeLabel(a) : a;
  const nb = t === TYPES.ARCHITECTURE ? normalizeArchitectureSubTypeLabel(b) : b;
  if (na === nb) return true;

  // 片方がスラッグだった場合に同値化（家具にも効く）
  return getSubtypeLabelFromSlug(t, a) === b || getSubtypeLabelFromSlug(t, b) === a;
}

// ✅ a/b がラベルかスラッグか不定でも、両方ラベル化して比較
function toGroupLabel(type, subTypeLabel, v) {
  const s = String(v ?? "");
  const label = getGroupLabelFromSlug(type, subTypeLabel, s);
  return label ?? decodeURIComponent(s);
}
...
export function isSameGroup(type, subTypeLabel, a, b) {
  ...
  const st =
    type === TYPES.ARCHITECTURE ? normalizeArchitectureSubTypeLabel(subTypeLabel) : subTypeLabel;

  return toGroupLabel(type, st, a) === toGroupLabel(type, st, b);
}

/** =========================
 *  任意：定義データの事故を減らす（浅いfreeze）
 *  ========================= */
Object.freeze(TYPES);
Object.freeze(SUBTYPES);
Object.freeze(ARCH_SUBTYPE_LEGACY_MAP);
Object.freeze(ARCH_SUBTYPE_CANONICAL_TO_LEGACY);
