...
import { getPlansColRef, getPlanDocRef, getItemsColRef } from "@layout/features/layout/paths/workspacePaths";

/**
 * shareId を生成（URLに載せてもOKな短めID）
 */
...
async function buildBaseTree(projectId, workspaceId, filterRootBaseId = null) {
    const plansCol = getPlansColRef({ projectId, workspaceId });
    if (!plansCol) return [];
    ...
    const plansWithLayouts = await Promise.all(
        relevantDocs.map(async (planData) => {
            const itemsCol = getItemsColRef({ projectId, workspaceId, planId: planData.id });
...
export async function saveLayoutLinkToPlan({
    ...
    const PROD_ORIGIN = "https://3dshapelayout.com";
