...
import {
  ...
  MenuItem, FormControl, InputLabel, Tooltip, CircularProgress, Divider,
} from "@mui/material";
...
import { getGlobalDb } from "@sekkeiya/global-panel";
...
const isOverdue = (dateStr) =>
  !!dateStr && new Date(dateStr + "T00:00:00") < new Date(new Date().toDateString());
...
function WeekView({ weekDays, schedules, onSlotClick, onEventClick }) {
  const byDay = useMemo(() => {
    const m = {};
    schedules.forEach(s => { if (s.dueDate) { (m[s.dueDate] = m[s.dueDate] || []).push(s); } });
    return m;
...
function DayView({ date, schedules, onSlotClick, onEventClick }) {
  const ds      = toDateStr(date);
          ...
          {timed.map(ev => {
            ...
            return (
              <Box key={ev.id} onClick={() => onEventClick(ev)}
                ...
                <Typography sx={{ fontSize:"0.68rem", color:"rgba(255,255,255,0.5)" }}>{ev.startTime}{ev.endTime?` – ${ev.endTime}`:""}</Typography>
              </Box>
            );
          })}
        </Box>
    ...
    <Box>
      {tasks.length === 0 && (
        <Box sx={{ py:4, textAlign:"center" }}>
          <Typography variant="body2" sx={{ color:"rgba(255,255,255,0.35)", fontWeight:600 }}>該当するタスクはありません</Typography>
        </Box>
      )}
      {tasks.map(task => {
...
function KanbanView({ tasks, onEdit, onDelete, onCycleStatus, onInlineSave }) {
  const [inlineCol,   setInlineCol]   = useState(null);  // col.id | null
  const [inlineTitle, setInlineTitle] = useState("");
  const [inlineType,  setInlineType]  = useState("manual");

  const commitInline = (colId) => {
  ...
  return (
    <Box sx={{ display:"flex", gap:2, overflowX:"auto", pb:1, px:"2px", "& ::-webkit-scrollbar":{ height:4 } }}>
      {KANBAN_COLS.map(col => {
              ...
              {inlineCol === col.id ? (
                <Box sx={{ bgcolor:"rgba(255,255,255,0.04)", border:"1px solid rgba(0,191,255,0.25)", borderRadius:2, p:1.125 }}>
                  <TextField
                    ...
                    sx={{ "& label.Mui-focused":{ color:"#00BFFF" } }}
                  />
                  {/* Type selector */}
  ...
  return (
    <Box sx={{ display:"flex", alignItems:"center", gap:1.5, px:1.5, py:0.875,
      ...
      <FormControl size="small" variant="standard" sx={{ flexShrink:0, minWidth:88 }}>
        <Select value={type} onChange={e => setType(e.target.value)} disableUnderline
...
const BLANK_SCH = { title:"", description:"", dueDate:"", startTime:"", endTime:"", type:"meeting", status:"upcoming" };

function ScheduleDialog({ open, initial, onClose, onSave }) {
  const [f, setF] = useState(BLANK_SCH);
  useEffect(() => { if (open) setF(initial ? { ...BLANK_SCH, ...initial } : BLANK_SCH); }, [open, initial]);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx:dlgPaper }}>
      ...
      <DialogContent sx={{ display:"flex", flexDirection:"column", gap:2, pt:"12px !important" }}>
        ...
        <FormControl size="small" fullWidth>
          ...
          <Select value={f.type} onChange={set("type")} label="種別" sx={selSx} MenuProps={{ PaperProps:{ sx:menuPpr } }}>
...
const BLANK_TASK = { title:"", description:"", type:"manual", priority:"medium", status:"todo", dueDate:"" };

function TaskDialog({ open, initial, onClose, onSave }) {
  const [f, setF] = useState(BLANK_TASK);
  useEffect(() => { if (open) setF(initial ? { ...BLANK_TASK, ...initial } : BLANK_TASK); }, [open, initial]);
  const set = k => e => setF(p => ({ ...p, [k]: e.target.value }));
      ...
      <DialogContent sx={{ display:"flex", flexDirection:"column", gap:2, pt:"12px !important" }}>
        ...
        <Box sx={{ display:"flex", gap:1.5 }}>
          ...
          <FormControl size="small" fullWidth>
            ...
            <Select value={f.priority} onChange={set("priority")} label="優先度" sx={selSx} MenuProps={{ PaperProps:{ sx:menuPpr } }}>
        ...
        <FormControl size="small" fullWidth>
          ...
          <Select value={f.status} onChange={set("status")} label="ステータス" sx={selSx} MenuProps={{ PaperProps:{ sx:menuPpr } }}>
...
export default function SchedulesTasksContent({ project, projectId }) {
  ...
  const [taskFilter, setTaskFilter] = useState("all");  // all|ai|manual|review|todo|done
  ...
  useEffect(() => {
    ...
    const us = onSnapshot(
      ...
      snap => { setSchedules(snap.docs.map(d=>({id:d.id,...d.data()}))); setLoadingS(false); },
      err  => { console.error(err); setLoadingS(false); }
    );
    const ut = onSnapshot(
      query(collection(db,"projects",projectId,"tasks"), orderBy("createdAt","asc")),
      snap => { setTasks(snap.docs.map(d=>({id:d.id,...d.data()}))); setLoadingT(false); },
      err  => { console.error(err); setLoadingT(false); }
    );
    return () => { us(); ut(); };
  }, [projectId]);
  ...
  const openNewSchedule = useCallback((date, time) => {
    setEditSch({ dueDate: date ? toDateStr(date) : toDateStr(new Date()), startTime: time||"" });
    setSchDlg(true);
  }, []);
  const openEditSchedule = useCallback(ev => { setEditSch(ev); setSchDlg(true); }, []);
  const openEditTask = useCallback(t => { setEditTask(t); setTaskDlg(true); }, []);

  // ── CRUD ──────────────────────────────────────────────────────────────────
  const uid = currentUser?.uid || "unknown";

  const saveSch = useCallback(async f => {
    const db = getGlobalDb();
    if (editSch?.id) {
  ...
  const deleteSch = useCallback(async id => {
    if (!window.confirm("この予定を削除しますか？")) return;
    await deleteDoc(doc(getGlobalDb(),"projects",projectId,"schedules",id));
  }, [projectId]);

  const toggleSchDone = useCallback(async item => {
    await updateDoc(doc(getGlobalDb(),"projects",projectId,"schedules",item.id),
      { status: item.status==="done"?"upcoming":"done", updatedAt:serverTimestamp() });
  }, [projectId]);

  const saveTask = useCallback(async f => {
    const db = getGlobalDb();
    if (editTask?.id) {
  ...
  const saveInlineTask = useCallback(async (title, type, status="todo") => {
    const db = getGlobalDb();
    await addDoc(collection(db,"projects",projectId,"tasks"), {
      title, type, priority:"medium", status, dueDate:"", description:"",
  ...
  const deleteTask = useCallback(async id => {
    ...
    await deleteDoc(doc(getGlobalDb(),"projects",projectId,"tasks",id));
  }, [projectId]);

  const cycleStatus = useCallback(async item => {
    await updateDoc(doc(getGlobalDb(),"projects",projectId,"tasks",item.id),
  ...
  const sectionHdr = (title, right) => (
    <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", mb:1.25 }}>
      <Typography sx={{ fontSize:"0.72rem", fontWeight:800, letterSpacing:1.2, color:"rgba(255,255,255,0.35)", textTransform:"uppercase" }}>
        {title}
      </Typography>
      {right}
    </Box>
      ...
      <Paper sx={{ bgcolor:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:3, mb:2, overflow:"hidden" }}>
        ...
        <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", px:2, py:1.25,
          ...
          <Box sx={{ display:"flex", gap:0.5, p:0.375, bgcolor:"rgba(0,0,0,0.25)", borderRadius:2, border:"1px solid rgba(255,255,255,0.07)" }}>
            {[
              { id:"month", Icon:CalendarMonthRoundedIcon,    label:"月" },
              { id:"week",  Icon:CalendarViewWeekRoundedIcon, label:"週" },
              { id:"day",   Icon:CalendarViewDayRoundedIcon,  label:"日" },
            ].map(({ id, Icon, label }) => (
          ...
          <Box sx={{ display:"flex", alignItems:"center", gap:1 }}>
            ...
            <Button size="small" onClick={() => setCalDate(new Date())}
              sx={{ ml:0.5, textTransform:"none", fontWeight:700, fontSize:"0.72rem", px:1.25, py:0.375, borderRadius:1.5, border:"1px solid rgba(255,255,255,0.15)", color:"rgba(255,255,255,0.7)", "&:hover":{ border:"1px solid rgba(255,255,255,0.35)", color:"#fff" } }}>
              今日
            </Button>
          </Box>
        ...
        <Box sx={{ p:{ xs:1, md:1.5 } }}>
          {loadingS ? (
            ...
            <MonthView year={year} month={month} schedules={schedules}
              ...
              onEventClick={ev => openEditSchedule(ev)}/>
          ) : calView==="week" ? (
      ...
      <Paper sx={{ bgcolor:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:3, overflow:"hidden" }}>
        ...
        <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", px:2, py:1.25,
          ...
          <Box sx={{ display:"flex", gap:0.5, p:0.375, bgcolor:"rgba(0,0,0,0.25)", borderRadius:2, border:"1px solid rgba(255,255,255,0.07)", flexShrink:0 }}>
            {[
              { id:"list",   Icon:ViewListRoundedIcon,   label:"リスト" },
              { id:"kanban", Icon:ViewKanbanRoundedIcon, label:"カンバン" },
            ].map(({ id, Icon, label }) => (
              <Button key={id} size="small" startIcon={<Icon sx={{ fontSize:"14px !important" }}/>}
          ...
          <Box sx={{ display:"flex", gap:0.75, flexWrap:"wrap", flex:1, alignItems:"center" }}>
            {[
              { key:"all",    label:"すべて" },
              { key:"ai",     label:"AI タスク" },
              { key:"manual", label:"自分で実行" },
              { key:"review", label:"確認事項" },
              { key:"todo",   label:"未完了" },
              { key:"done",   label:"完了済み" },
            ].map(({ key, label }) => (
          ...
          {/* Right: + New (list mode: inline row / kanban mode: each column has own add) */}
          {taskView === "list" && (
            <Button size="small" startIcon={<AddRoundedIcon sx={{ fontSize:"14px !important" }}/>}
